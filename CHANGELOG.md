## 3.0.1 - UI Polish & Release Prep

### ✨ New Features

- Matrix theme option with accent-driven visuals
- Sidebar Matrix rain animation when tasks are active

### 🛠️ Improvements

- Rounded app icons across all platforms (macOS/Windows/Linux)
- Accent‑tinted sidebar logo and header layout polish
- Added project attribution notice (Auto‑Claude)

### 🧹 Maintenance

- Version bump to 3.0.1 across root and frontend packages
- Removed local settings from repo; added NOTICE

## 2.8.0 - Scope Guardrails & Post‑Code Testing

### ✨ New Features

- Scope Contract system (schema + rules) with preflight review and senior approval
- Code‑scope enforcement to block writes outside allowed paths
- Post‑code test runner with gating and UI feedback

### 🛠️ Improvements

- UI indicators for scope guardrails and post‑code tests
- Block test commands during coding phase and bash during manual verification
- Auto‑install backend venv when missing before backend tests
- Preflight error surfacing and normalized paths

### 🐛 Bug Fixes

- Clean sandbox data directories after copy
- Restore worktree write guard after legacy cleanup
- Enable full top‑panel drag regions (Electron drag/no‑drag)
...
- Removed legacy path from auto-iflow source detection
...
- fix: remove legacy path from auto-iflow source detection (#148) by @Joris Slagter in f96c6301
...
- Default database path unified to ~/.auto-iflow/memories for consistency
...
- fix: unify default database path to ~/.auto-iflow/memories by @AndyMik90 in 684e3f9
...
- auto-iflow: subtask-1-1 - Add projectPath prop to PreviewPanel and implement custom img component by @AndyMik90 in e6529e0
...
- Required GitHub setup flow after Auto-iFlow initialization to ensure proper configuration
...
- Removed redundant auto-iflow/specs directory structure
- Untracked .auto-iflow directory to respect gitignore rules
...
- feat: add required GitHub setup flow after Auto-iFlow initialization by @AndyMik90 in 03ccce5
- chore: remove redundant auto-iflow/specs directory by @AndyMik90 in 64d5170
- chore: untrack .auto-iflow directory (should be gitignored) by @AndyMik90 in 0710c13
...
- auto-iflow: subtask-2-3 - Refine visual drop zone feedback to be more subtle by @AndyMik90 in 6cff442

- auto-iflow: subtask-2-1 - Remove showFiles auto-expand on draft restore by @AndyMik90 in 12bf69d

- auto-iflow: subtask-1-3 - Create an always-visible referenced files section by @AndyMik90 in 3818b46

- auto-iflow: subtask-1-2 - Add drop zone wrapper around main modal content area by @AndyMik90 in 219b66d

- auto-iflow: subtask-1-1 - Remove Reference Files toggle button by @AndyMik90 in 4e63e85
...
- feat: auto-iflow: subtask-1-1 - Restructure SortableFeatureCard badge layout by @AndyMik90 in dc886dc
...
- Updated README to clarify Auto-iFlow's role as an AI coding companion
...
- Removed .auto-iflow and _bmad-output from git tracking (already in .gitignore)
...
- chore: Remove .auto-iflow from tracking (already in .gitignore) by @AndyMik90 in 87f353c
...
- docs: Revise README.md to enhance clarity and focus on Auto-iFlow's capabilities by @AndyMik90 in f9ef7ea
...
- Removed obsolete security configuration file (.auto-iflow-security.json)
