...
from init import AUTO_BUILD_BRANCH_NAMESPACE
...
from spec.post_code_tests import get_test_plan
...
async def run_autonomous_agent(
    ...
    Args:
        ...
        spec_dir: Directory containing the spec (.auto-iflow/specs/001-name/)
    ...
    os.environ[PROJECT_DIR_ENV_VAR] = project_dir_value
    spec_dir_value = str(spec_dir.resolve())
    os.environ[SPEC_DIR_ENV_VAR] = spec_dir_value

    intake = load_task_intake(spec_dir) or {}
    os.environ[TASK_TYPE_ENV_VAR] = str(intake.get("task_type", "code"))
    os.environ[NOISE_PROFILE_ENV_VAR] = str(intake.get("noise_profile", "medium"))

    def _set_test_command_guard() -> None:
        os.environ[BLOCK_TEST_COMMANDS_ENV_VAR] = "true"
        test_plan = get_test_plan(spec_dir)
        if test_plan:
            os.environ[TEST_PLAN_ENV_VAR] = json.dumps(test_plan)
        else:
            os.environ.pop(TEST_PLAN_ENV_VAR, None)

    def _clear_test_command_guard() -> None:
        os.environ.pop(BLOCK_TEST_COMMANDS_ENV_VAR, None)
        os.environ.pop(TEST_PLAN_ENV_VAR, None)
    ...
    allowed_dirs, scope_error = resolve_scope_write_dirs(spec_dir, project_dir)
    if scope_error:
        print_status(f"Scope enforcement blocked: {scope_error}", "error")
        if task_logger:
            task_logger.log_error(
                f"Scope enforcement blocked: {scope_error}", LogPhase.CODING
            )
        status_manager.update(state=BuildState.ERROR)
        return

    print_status(
        f"Scope enforcement enabled ({len(allowed_dirs)} allowed paths)", "info"
    )
    ...
    while True:
        ...
        session_settings = {
            "session_id": f"{spec_dir.name}:{subtask_id or 'planner'}:{iteration}",
            "fresh_context": True,
        }
        client = create_iflow_client(
            project_dir,
            phase_model,
            enable_thinking=phase_thinking_budget is not None,
        ...
        if first_run:
            ...
            if (
                ...
                or context.get("files_to_modify")
                or context.get("progress_notes")
            ):
            ...
            # Detect manual verification subtasks (skip command execution)
            verification = next_subtask.get("verification") or {}
            manual_verification = verification.get("type") == "manual" or verification.get(
                "step_type"
            ) == "manual"
            if manual_verification:
                manual_instructions = (
                    verification.get("instructions")
                    or verification.get("scenario")
                    or "Manual verification required. Provide steps for a human reviewer."
                )
                prompt = (
                    "IMPORTANT: This is a MANUAL verification step. "
                    "Do NOT run bash/local commands or start servers. "
                    "Provide clear manual verification steps only.\n\n"
                    + prompt
                )
                if task_logger:
                    task_logger.log_with_detail(
                        "Manual verification required - command execution disabled",
                        manual_instructions,
                        LogEntryType.INFO,
                        LogPhase.CODING,
                        subphase="MANUAL VERIFICATION",
                    )
        ...
        manual_guard_enabled = False
        if not first_run and next_subtask:
            verification = next_subtask.get("verification") or {}
            manual_guard_enabled = verification.get("type") == "manual" or verification.get(
                "step_type"
            ) == "manual"

        if manual_guard_enabled:
            os.environ[MANUAL_VERIFICATION_ENV_VAR] = "true"
            if subtask_id:
                os.environ[MANUAL_VERIFICATION_SUBTASK_ENV_VAR] = subtask_id

        test_guard_enabled = True
        if test_guard_enabled:
            _set_test_command_guard()
        ...
        try:
            ...
            if manual_guard_enabled:
                os.environ.pop(MANUAL_VERIFICATION_ENV_VAR, None)
                os.environ.pop(MANUAL_VERIFICATION_SUBTASK_ENV_VAR, None)
            if test_guard_enabled:
                _clear_test_command_guard()
        ...
        if status == "complete":
            task_intake = load_task_intake(spec_dir) or {}
            task_type = (task_intake.get("task_type") or "code").strip().lower()
            ...
            if task_type != "code":
                ...
                if task_logger:
                    ...
                    task_logger.log_info(
                        f"Validation skipped: non-code task (task_type={task_type})",
                        LogPhase.VALIDATION,
                    )
                    task_logger.end_phase(
                        LogPhase.VALIDATION,
                        success=True,
                        message=f"Validation skipped for non-code task (type={task_type})",
                    )
                ...
                if plan_file.exists():
                    try:
                        ...
                        print_status(
                            "Warning: implementation_plan.json is invalid JSON",
                            "warning",
                        )

                emit_phase(
                    ExecutionPhase.COMPLETE,
                    f"Non-code task complete (task_type={task_type})",
                )

                if linear_task and linear_task.task_id:
                    await linear_build_complete(spec_dir)
                    print_status(
                        "Linear notified: build complete (non-code)", "success"
                    )
    ...
    if completed < total:
        ...
        content = [
            ...
            f"  1. Review the {AUTO_BUILD_BRANCH_NAMESPACE}/* branch",
            "  2. Run manual tests",
            "  3. Merge to main",
        ]
