...
from .models import (
    AGENT_CONFIGS,
    AUTO_BUILD_MCP_ALIASES,
    AUTO_BUILD_MCP_SERVER,
    CONTEXT7_TOOLS,
    ELECTRON_TOOLS,
    GRAPHITI_MCP_TOOLS,
    LINEAR_TOOLS,
...
def get_allowed_tools(
    ...
    # Add auto-build tools ONLY if the MCP server is available
    # This prevents allowing tools that won't work because the server isn't running
    auto_build_server_ids = {AUTO_BUILD_MCP_SERVER, *AUTO_BUILD_MCP_ALIASES.keys()}
    if auto_build_server_ids.intersection(required_servers) and is_tools_available():
        tools.extend(config.get("auto_iflow_tools", []))
...
def _get_mcp_tools_for_servers(servers: list[str]) -> list[str]:
    ...
    for server in servers:
        ...
        # auto-iflow tools are already added via config["auto_iflow_tools"]
