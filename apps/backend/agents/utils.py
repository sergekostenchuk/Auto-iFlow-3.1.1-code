...
def sync_spec_to_source(spec_dir: Path, source_spec_dir: Path | None) -> bool:
    ...
    IMPORTANT: Since .auto-iflow is gitignored, this sync happens to the
    local filesystem regardless of what branch the user is on. The worktree
    may be on a different branch (e.g., auto-iflow + "/093-task"), but the sync
    target is always the main project's .auto-iflow/specs/ directory.
