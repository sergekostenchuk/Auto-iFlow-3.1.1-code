...
Uses the iFlow SDK (same as the rest of the system) for extraction.
...
# Check for iFlow SDK availability
try:
...
DEFAULT_EXTRACTION_MODEL = "glm-4.7"
...
def is_extraction_enabled() -> bool:
    ...
    # Extraction requires iFlow SDK and authentication
...
def get_extraction_model(project_dir: Path | None = None) -> str:
    """Get the model to use for insight extraction."""
    override = os.environ.get("INSIGHT_EXTRACTOR_MODEL")
    try:
        auto_build_path = resolve_auto_build_dir(project_dir).name if project_dir else None
        ...
        return override or DEFAULT_EXTRACTION_MODEL
...
async def run_insight_extraction(
    ...
    """
    Run the insight extraction using iFlow.
    ...
    """
    if not IFLOW_SDK_AVAILABLE:
        logger.warning("iFlow SDK not available, skipping insight extraction")
        return None

    if not has_iflow_auth():
        logger.warning("No iFlow authentication found, skipping insight extraction")
        return None

    # Ensure SDK can find the token
    ensure_iflow_auth_env()

    model = get_extraction_model(project_dir)
