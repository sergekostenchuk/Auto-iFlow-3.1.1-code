...
def _run_build_with_optional_ralph_loop(
    *,
    working_dir: Path,
    spec_dir: Path,
    model: str | None,
    ...
    """
    Execute build loop with optional Ralph-loop iterations.

    Returns:
        True if QA approved, otherwise False.
    """
    ...
    def _run_build_iteration() -> bool:
        ...
        if not skip_qa and should_run_qa_func(spec_dir):
            print("\n" + "=" * 70)
            ...
            try:
                ...
                if qa_approved:
                    ...
                    print(
                        f"\nResume QA: python apps/backend/run.py --spec {spec_dir.name} --qa\n"
                    )

                # Sync implementation plan to main project after QA
                # This ensures the main project has the latest status (human_review)
                if sync_spec_func(spec_dir, source_spec_dir):
                    debug_info(
                        "run.py", "Implementation plan synced to main project after QA"
                    )
            except KeyboardInterrupt:
                print("\n\nQA validation paused.")
                print(f"Resume: python apps/backend/run.py --spec {spec_dir.name} --qa")
                qa_approved = False
        return qa_approved

    qa_approved = False
    ralph_iteration = 0
    ralph_loop_max = max(1, ralph_loop_max)
    while True:
        ralph_iteration += 1
        if ralph_loop_enabled:
            print(f"\nRalph-loop iteration {ralph_iteration}/{ralph_loop_max}")

        qa_approved = _run_build_iteration()

        if not ralph_loop_enabled or qa_approved:
            break

        if ralph_iteration >= ralph_loop_max:
            print("\nRalph-loop reached max iterations without QA approval.")
            break

        print("\nRalph-loop continuing: QA not approved, re-running build...\n")
...
def handle_build_command(
    ...
    Args:
        ...
        model: Optional model override (may be overridden by task metadata)
    ...
    from core.model_routing import write_resolved_model_snapshot
    ...
    try:
        write_resolved_model_snapshot(spec_dir)
    except Exception as exc:
        debug_info("run.py", f"Failed to write resolvedModel snapshot: {exc}")

    # Get task metadata (for optional Ralph-loop and other settings)
    task_metadata = load_task_metadata(spec_dir) or {}
    ralph_loop_enabled = bool(task_metadata.get("ralphLoop"))
    ralph_loop_max = task_metadata.get("ralphLoopMax", 3)
    try:
        ralph_loop_max = int(ralph_loop_max)
    ...
    if ralph_loop_enabled:
        print(f"Ralph-loop: ENABLED (max iterations: {ralph_loop_max})")
    print()
        ...
        if force_bypass_approval:
            ...
            content.extend(
                [
                    ...
                    f"  python apps/backend/review/main.py --spec-dir {spec_dir}",
                    "",
    ...
    try:
        ...
        qa_approved = _run_build_with_optional_ralph_loop(
            ...
            ralph_loop_max=ralph_loop_max,
            run_agent_func=run_autonomous_agent,
            run_qa_func=run_qa_validation_loop,
            should_run_qa_func=should_run_qa,
            sync_spec_func=sync_spec_to_source,
