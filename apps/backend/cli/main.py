"""
Auto-iFlow CLI - Main Entry Point
=================================

Command-line interface for the Auto-iFlow autonomous coding framework.
"""
...
from .consilium_commands import handle_consilium_command


def parse_args() -> argparse.Namespace:
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(
        description="Auto-iFlow Framework - Autonomous multi-session coding agent",
...
Environment Variables:
  ...
  AUTO_BUILD_MODEL         Override default model (optional)
    ...
    parser.add_argument(
        "--migrate-task-metadata",
        action="store_true",
        help="Migrate legacy task_metadata.json to modelRouting (project-wide)",
    )
    parser.add_argument(
        "--specs-dir",
        type=Path,
        default=None,
        help="Override specs directory for task metadata migration",
    )
    ...
    parser.add_argument(
        "--consilium",
        action="store_true",
        help="Run Consilium multi-agent orchestrator (Planner, Executor, Reviewer)",
    )
    parser.add_argument(
        "--task",
        type=str,
        default=None,
        help="Task description for Consilium (use with --consilium)",
    )
...
def main() -> None:
    ...
    if args.migrate_task_metadata:
        from core.task_metadata_migration import migrate_task_metadata_tree
        from spec.pipeline import get_specs_dir

        print_banner()
        specs_dir = args.specs_dir or get_specs_dir(project_dir, ensure_write=True)
        result = migrate_task_metadata_tree(specs_dir)
        print(f"\nSpecs directory: {specs_dir}")
        print(
            "Migrated task_metadata.json files: "
    ...
    # Handle --consilium command
    if args.consilium:
        if not args.task:
            print("\nError: --task is required with --consilium")
            print("\nUsage:")
            print('  python run.py --consilium --task "Create a snake game"')
            sys.exit(1)
        handle_consilium_command(
