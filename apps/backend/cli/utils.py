...
Shared utility functions for the Auto-iFlow CLI.
"""
...
from core.auth import get_iflow_auth_state, has_iflow_auth
...
def setup_environment() -> Path:
    ...
    # Load .env file - check backend and dev locations
    env_file = script_dir / ".env"
    dev_env_file = script_dir.parent / "dev" / AUTO_BUILD_BRANCH_NAMESPACE / ".env"
    if env_file.exists():
...
def find_spec(project_dir: Path, spec_identifier: str) -> Path | None:
    ...
    data_dir = resolve_auto_build_dir(project_dir)
    worktree_base = data_dir / "worktrees" / "tasks"
    if worktree_base.exists():
        ...
        for worktree_dir in worktree_base.iterdir():
            if worktree_dir.is_dir() and worktree_dir.name.startswith(
                ...
                spec_in_worktree = (
                    worktree_dir / data_dir.name / "specs" / worktree_dir.name
                )
                if (
...
def validate_environment(spec_dir: Path) -> bool:
    ...
    # Check for iFlow auth
    if not has_iflow_auth():
        print("Error: iFlow authentication required")
        print("\nConfigure an iFlow API key in Settings → Integrations or sign in via web login.")
        valid = False
    else:
        state = get_iflow_auth_state()
        if state.auth_type:
            print(f"Auth: {state.auth_type}")
        if state.base_url:
            print(f"API Endpoint: {state.base_url}")
    ...
    if is_linear_enabled():
        ...
        project_dir = spec_dir.parent.parent  # data_dir/specs/001-name -> project root
