...
def _resolve_spec_branch(project_dir: Path, spec_name: str) -> str:
    """Resolve spec branch name (auto-iflow preferred, legacy fallback)."""
    manager = WorktreeManager(project_dir)
    return manager.get_branch_name(spec_name)
...
def _generate_and_save_commit_message(project_dir: Path, spec_name: str) -> None:
    ...
    try:
        ...
        from init import resolve_auto_build_dir_for_write
        ...
        if commit_message:
            ...
            data_dir = resolve_auto_build_dir_for_write(project_dir)
            spec_dir = data_dir / "specs" / spec_name
...
def handle_list_worktrees_command(project_dir: Path) -> None:
    ...
    if not worktrees:
        ...
        print("-" * 70)
        print()
        print("  To merge:   python run.py --spec <name> --merge")
        print("  To review:  python run.py --spec <name> --review")
        print("  To discard: python run.py --spec <name> --discard")
        print()
        print(
            "  To cleanup all worktrees: python run.py --cleanup-worktrees"
        )
    print()
...
def _check_git_merge_conflicts(project_dir: Path, spec_name: str) -> dict:
    ...
    spec_branch = _resolve_spec_branch(project_dir, spec_name)
    result = {
        "has_conflicts": False,
        "conflicting_files": [],
        "needs_rebase": False,
