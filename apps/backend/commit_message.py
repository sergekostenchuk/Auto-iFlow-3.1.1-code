...
async def _call_iflow(prompt: str) -> str:
    """Call iFlow for commit message generation.
...
from phase_config import resolve_model
    ...
    if not has_iflow_auth():
        logger.warning("No iFlow authentication found")
        return ""
    ...
    try:
        ...
        logger.error(f"iFlow call failed: {e}")
    ...
    try:
        ...
        if loop and loop.is_running():
            ...
            with concurrent.futures.ThreadPoolExecutor() as pool:
                result = pool.submit(lambda: asyncio.run(_call_iflow(prompt))).result()
        else:
            result = asyncio.run(_call_iflow(prompt))
...
async def generate_commit_message(
    ...
    # Call Claude
    try:
        result = await _call_iflow(prompt)
        if result:
            return result
    except Exception as e:
        logger.error(f"Failed to generate commit message: {e}")
