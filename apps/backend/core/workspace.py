...
gets its own isolated worktree in .auto-iflow/worktrees/tasks/{spec-name}/.
...
def resolve_spec_branch(project_dir: Path, spec_name: str) -> str:
    """Resolve spec branch name (auto-iflow only)."""
...
def merge_existing_build(
    ...
    Called when user runs: python apps/backend/run.py --spec X --merge
    ...
    if not worktree_path:
        ...
        print(highlight(f"  python apps/backend/run.py --spec {spec_name}"))
        return False
    ...
    spec_branch = resolve_spec_branch(project_dir, spec_name)
    ...
    if current_branch == spec_branch:
        ...
        print(highlight(f"  python apps/backend/run.py --spec {spec_name} --merge"))
        return False
        ...
        if smart_result is not None:
            ...
            if smart_result.get("success"):
                ...
                if had_conflicts or files_merged or ai_assisted:
                    ...
                    # User can delete with: python apps/backend/run.py --spec <name> --discard
    ...
    if success_result:
        ...
        if no_commit:
            ...
            print(muted(f"  python apps/backend/run.py --spec {spec_name} --discard"))
        else:
    ...
    try:
        ...
        if merge_tree_result.returncode != 0:
            ...
            for line in output.split("\n"):
                if "CONFLICT" in line:
                    ...
                    if match:
                        ...
                        # Skip data directory files - they should never be merged
                        if (
                            file_path
                            and file_path not in result["conflicting_files"]
                            and not _is_auto_build_file(file_path)
                        ):
            ...
            if not result["conflicting_files"]:
                ...
                # Filter out data directory files - they should never be merged
                conflicting = main_files & spec_files
                result["conflicting_files"] = [
                    f for f in conflicting if not _is_auto_build_file(f)
                ]
...
async def _merge_file_with_ai_async(
    ...
    async with semaphore:
        try:
            ...
            try:
                model_id, _, thinking_budget = resolve_model(
                    feature="merge",
                ...
                model_id = model_override or "glm-4.7"
