    ...
    if choice == WorkspaceChoice.TEST:
        ...
        if staging_path:
            ...
            if worktree_path:
                ...
                print(
                    highlight(
                        f"  cd {resolve_auto_build_dir(project_dir) / 'worktrees' / 'tasks' / spec_name}"
                    )
                )
        ...
        print(muted(f"  python apps/backend/run.py --spec {spec_name} --review"))
        print()
...
def review_existing_build(project_dir: Path, spec_name: str) -> bool:
    ...
    Called when user runs: python apps/backend/run.py --spec X --review
...
def discard_existing_build(project_dir: Path, spec_name: str) -> bool:
    ...
    Called when user runs: python apps/backend/run.py --spec X --discard
