...
from init import DEFAULT_AUTO_BUILD_DIR, resolve_auto_build_dir
...
def get_existing_build_worktree(project_dir: Path, spec_name: str) -> Path | None:
    ...
    """
    # New path first (data directory)
    data_dir = resolve_auto_build_dir(project_dir)
    new_path = data_dir / "worktrees" / "tasks" / spec_name
    if new_path.exists():
        return new_path
...
def get_changed_files_from_branch(
    ...
    Args:
        ...
        exclude_auto_build: If True, exclude data directory files (default True)
    ...
    if result.returncode == 0:
        for line in result.stdout.strip().split("\n"):
            if line:
                ...
                if len(parts) == 2:
                    file_path = parts[1]
                    # Exclude data directory files from merge
                    if exclude_auto_build and _is_auto_build_file(file_path):
                        continue
...
def _is_auto_build_file(file_path: str) -> bool:
    """Check if a file is in the data directory (auto-iflow).
