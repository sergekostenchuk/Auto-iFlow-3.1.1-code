...
class WorkspaceMode(Enum):
    """How the workspace should operate."""
...
class MergeLock:
    ...
    Uses a lock file in the project data directory to ensure only one merge operation
    ...
    def __init__(self, project_dir: Path, spec_name: str):
        ...
        data_dir = project_dir / ".auto-iflow"
...
class SpecNumberLock:
    ...
    def __init__(self, project_dir: Path):
        self.project_dir = project_dir
        self.data_dir = project_dir / ".auto-iflow"
    ...
    def get_next_spec_number(self) -> int:
        ...
        main_specs_dir = self.data_dir / "specs"
        max_number = max(max_number, self._scan_specs_dir(main_specs_dir))

        # 2. Scan all worktree specs
        worktrees_dir = self.data_dir / "worktrees" / "tasks"
        if worktrees_dir.exists():
            for worktree in worktrees_dir.iterdir():
                if worktree.is_dir():
                    worktree_specs = worktree / self.data_dir.name / "specs"
