...
def choose_workspace(
    ...
    """
    Let user choose where the agent should work.
...
def copy_spec_to_worktree(
    ...
    # Use data_dir/specs/{spec_name}/ as the standard location
    target_spec_dir = worktree_path / data_dir_name / "specs" / spec_name
...
def setup_workspace(
    ...
    # Ensure data directory is in the worktree's .gitignore
    # This is critical because the worktree inherits .gitignore from the base branch,
    # which may not have data directory entries if that change wasn't committed/pushed.
    # Without this, spec files would be committed to the worktree's branch.
    from init import ensure_gitignore_entry

    if ensure_gitignore_entry(worktree_info.path, f"{data_dir_name}/"):
        debug(MODULE, f"Added {data_dir_name}/ to worktree's .gitignore")
    ...
    if source_spec_dir and source_spec_dir.exists():
        localized_spec_dir = copy_spec_to_worktree(
            source_spec_dir, worktree_info.path, spec_name, data_dir_name
        )
