...
Uses iFlow agents to generate ideas of different types:
- Code improvements
- UI/UX improvements
- Documentation gaps
- Security hardening
...
class IdeationGenerator:
    ...
    def __init__(
        ...
        self.thinking_override = thinking_level
        self.model, self.thinking_level, self.thinking_budget = (
            self._resolve_model_config()
        )
        self.max_ideas_per_type = max_ideas_per_type
        self.prompts_dir = Path(__file__).parent.parent / "prompts"

    def _resolve_model_config(self) -> tuple[str, str, int | None]:
        model_override = (self.model_override or "").strip()
        if model_override.lower() in ("", "auto", "default", "profile"):
            model_override = None

        thinking_override = (self.thinking_override or "").strip()
    ...
    def _resolve_model_for_client(self) -> str:
        return self.model
    ...
    async def run_agent(
        ...
        client = create_client(
            self.project_dir,
            self.output_dir,
            self._resolve_model_for_client(),
