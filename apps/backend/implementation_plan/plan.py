...
class ImplementationPlan:
    ...
    post_code_tests: dict | None = None
    ...
    def to_dict(self) -> dict:
        ...
        if self.post_code_tests:
            result["post_code_tests"] = self.post_code_tests
        return result
    ...
    def from_dict(cls, data: dict) -> "ImplementationPlan":
        ...
        return cls(
            ...
            post_code_tests=data.get("post_code_tests"),
        )
    ...
    def update_status_from_subtasks(self):
        ...
        if completed_count == total_count:
            # All subtasks completed - check if QA approved and tests passed
            post_code_status = None
            if isinstance(self.post_code_tests, dict):
                post_code_status = self.post_code_tests.get("status")
            tests_passed = post_code_status == "passed"

            if self.qa_signoff and self.qa_signoff.get("status") == "approved" and tests_passed:
                self.status = "human_review"
                self.planStatus = "review"
            else:
                # All subtasks done, waiting for QA or test pass
