...
Environment Variables:
    ...
    # Database
    GRAPHITI_DATABASE: Graph database name (default: auto_iflow_memory)
    GRAPHITI_DB_PATH: Database storage path (default: ~/.auto-iflow/graphs)
...
DEFAULT_DATABASE = "auto_iflow_memory"
DEFAULT_DB_PATH = "~/.auto-iflow/graphs"
...
def _find_existing_database(base_path: Path) -> str | None:
    if (base_path / DEFAULT_DATABASE).exists():
        return DEFAULT_DATABASE
    return None


def _resolve_default_database(db_path: str | None = None) -> str:
    if db_path:
        existing = _find_existing_database(Path(db_path).expanduser())
        if existing:
            return existing
        return DEFAULT_DATABASE

    default_candidate = Path(DEFAULT_DB_PATH).expanduser()
    existing = _find_existing_database(default_candidate)
    if existing:
        return existing
    return DEFAULT_DATABASE
...
class GraphitiConfig:
    ...
    def from_env(cls) -> "GraphitiConfig":
        ...
        db_path = os.environ.get("GRAPHITI_DB_PATH", _resolve_default_db_path())
        database = os.environ.get("GRAPHITI_DATABASE", _resolve_default_database(db_path))
    ...
    def get_provider_specific_database_name(self, base_name: str = None) -> str:
        ...
        Returns:
            Database name with provider signature (e.g., "auto_iflow_memory_ollama_768")
        """
...
@dataclass
class GraphitiState:
    """State of Graphiti integration for an Auto-iFlow spec."""
