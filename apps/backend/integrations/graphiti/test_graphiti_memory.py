...
# Add Auto-iFlow to path
auto_iflow_dir = Path(__file__).parent.parent.parent
sys.path.insert(0, str(auto_iflow_dir))

# Load .env file
try:
    from dotenv import load_dotenv

    env_file = auto_iflow_dir / ".env"
    if env_file.exists():
...
async def test_ollama_embeddings() -> bool:
    ...
    try:
        ...
        test_text = (
            "This is a test embedding for Auto-iFlow memory system using LadybugDB."
        )
...
async def main():
    ...
    parser.add_argument(
        "--db-path",
        default=os.path.expanduser("~/.auto-iflow/graphs"),
        help="Database path",
    )
    parser.add_argument(
        "--database",
    ...
    print(
        "    python integrations/graphiti/test_graphiti_memory.py --database auto_iflow_memory"
    )
    print()
