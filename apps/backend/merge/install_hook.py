...
Usage:
    python -m auto_iflow.merge.install_hook [--project-path /path/to/project]
"""
...
# Skip if we're in a worktree (auto-iflow branches)
if [[ "$BRANCH" == "main" ]] || [[ "$BRANCH" == "master" ]]; then
    ...
    if [[ -d ".git" ]]; then
        ...
        ($PYTHON -m auto_iflow.merge.tracker_cli notify-commit "$COMMIT_HASH" 2>/dev/null &) &
