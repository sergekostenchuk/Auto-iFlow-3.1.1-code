...
Usage:
    python -m auto_iflow.merge.tracker_cli notify-commit <hash>
    python -m auto_iflow.merge.tracker_cli show-timeline <file_path>
    python -m auto_iflow.merge.tracker_cli show-drift <task_id>
"""
...
def find_project_root() -> Path:
    """Find the project root by looking for a data dir or .git directory."""
    current = Path.cwd()

    # Walk up until we find a data dir or .git
    while current != current.parent:
        if resolve_auto_build_dir(current).exists() or (current / ".git").exists():
            return current
