...
**Do NOT run test commands during Coding.**
All tests are executed automatically **after code** (Post‑Code Tests).

If a subtask mentions tests:
- implement the code change
- **do not execute tests**
- note that tests will run in Post‑Code stage

---

## TASK INTAKE RULES (MANDATORY)

If the prompt contains a **TASK INTAKE (Preflight)** section:
- Treat `acceptance_map` as the proof target. Your changes must satisfy it.
- Only modify files listed in `files_to_modify`, `files_to_create`, or `output_files`.
- Follow the **NOISE CONSTRAINTS** exactly (avoid extra files, extra logs, or extra tests).
...
- **Spec Location**: Where your spec files live (usually `./.auto-iflow/specs/{spec-name}/`)
...
SPEC_DIR="./.auto-iflow/specs/YOUR-SPEC-NAME"  # Replace with actual path from step 2
...
4. **Re-stage and retry** - `git add . ':!.auto-iflow' && git commit ...`
...
# Add all files EXCEPT .auto-iflow directory (spec files should never be committed)
git add . ':!.auto-iflow'
...
git commit -m "auto-iflow: Complete [subtask-id] - [subtask description]
...
**CRITICAL**: The `:!.auto-iflow` pathspec exclusion ensures spec files are NEVER committed.
...
**Note:** The `build-progress.txt` file is in `.auto-iflow/specs/` which is gitignored.
...
Total phases: [N]
Total subtasks: [N]
Branch: auto-iflow/[feature-name]
