...
Do **NOT** add test commands inside subtask verification.

- Put test commands in `Scope Contract -> test_plan` (post‑code only).
- In new subtasks, keep `verification` focused on code correctness,
  **not** test commands.

If you must reference tests in a subtask, use:
...
Next Steps:
Run `python auto-iflow/run.py --spec [SPEC_NUMBER]` to continue with new subtasks.
...
To continue building:
  python auto-iflow/run.py --spec [SPEC_NUMBER]
...
This spec has never been built. Please run:
  python auto-iflow/run.py --spec [NUMBER]
