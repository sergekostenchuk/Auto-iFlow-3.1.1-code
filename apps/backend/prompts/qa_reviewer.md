...
- **If task_type != code**: do NOT run tests or builds. Validate the document outputs only.
- Proof Gate is enforced after QA: ensure `proofs.json` covers the acceptance criteria.
...
# Note: qa_report.md and implementation_plan.json are in .auto-iflow/specs/ (gitignored)
...
# Note: QA_FIX_REQUEST.md and implementation_plan.json are in .auto-iflow/specs/ (gitignored)
# Do NOT commit them - the framework tracks QA status automatically
