...
from .prompts import get_noise_constraints
...
def get_relative_spec_path(spec_dir: Path, project_dir: Path) -> str:
    ...
    Returns:
        Relative path string (e.g., "./.auto-iflow/specs/003-new-spec")
    """
    try:
        ...
        return f"./.auto-iflow/specs/{spec_dir.name}"
...
def generate_subtask_prompt(
    ...
    intake_section = _format_task_intake_section(spec_dir)
    if intake_section:
        sections.append(intake_section)
...
1. **Read `build-progress.txt`** to understand current progress and blockers
2. **Read the pattern files** to understand code style and conventions
3. **Read the files to modify** (if any) to understand current implementation
4. **Implement the subtask** following the patterns exactly
5. **Run verification** and fix any issues
6. **Commit your changes:**
   ```bash
   git add .
   git commit -m "auto-iflow: {subtask_id} - {description[:50]}"
   ```
7. **Update the plan** - set this subtask's status to "completed" in implementation_plan.json
8. **Append a progress note** to build-progress.txt with what changed and result
...
- Always document progress or blockers in build-progress.txt
""")
...
def generate_planner_prompt(spec_dir: Path, project_dir: Path | None = None) -> str:
    ...
    if project_dir is None:
        # Infer: spec_dir is typically project/.auto-iflow/specs/XXX
...
"""
    ...
    return header + _format_task_intake_section(spec_dir) + prompt
    ...
    if progress_path.exists():
        try:
            ...
            if len(lines) > max_file_lines:
                content = "\n".join(lines[-max_file_lines:])
                content = (
                    f"... (truncated, showing last {max_file_lines} lines)\n"
                    + content
                )
            else:
                content = "\n".join(lines)
            context["progress_notes"] = content.strip()
        except Exception:
            context["progress_notes"] = "(Could not read build-progress.txt)"
...
def format_context_for_prompt(context: dict) -> str:
    ...
    if context.get("progress_notes"):
        sections.append("## Progress Notes (build-progress.txt)\n")
