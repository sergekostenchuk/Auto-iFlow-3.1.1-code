...
def _load_task_intake(spec_dir: Path) -> dict:
...
def get_noise_constraints(noise_profile: str | None) -> str:
    """Return noise control constraints for the given noise profile."""
    profile = (noise_profile or "").lower().strip()
    if profile == "low":
        return (
            "Noise profile: LOW\n"
            "- Keep output concise. No long reports.\n"
            "- Do NOT run tests unless explicitly required.\n"
            "- Avoid extra files or refactors outside scope.\n"
        )
    if profile == "medium":
        return (
            "Noise profile: MEDIUM\n"
            "- Keep output focused; short validation only.\n"
            "- Run only required tests.\n"
            "- Limit scope to listed files.\n"
        )
    return (
        "Noise profile: HIGH\n"
        "- Thorough validation allowed.\n"
        "- Run required tests and document results.\n"
        "- Still avoid scope creep.\n"
    )


def _format_task_intake_section(spec_dir: Path) -> str:
    intake = _load_task_intake(spec_dir)
    if not intake:
        return ""

    task_type = intake.get("task_type", "code")
    noise_profile = intake.get("noise_profile", "medium")
    acceptance_map = intake.get("acceptance_map", [])
    output_files = intake.get("output_files", [])
    tests_to_run = intake.get("tests_to_run", [])

    constraints = get_noise_constraints(noise_profile)
...
def get_planner_prompt(spec_dir: Path) -> str:
    ...
    if not prompt_file.exists():
        raise FileNotFoundError(
            ...
            "Make sure the apps/backend/prompts/planner.md file exists."
        )
...
The project root is the parent of .auto-iflow. Implement code in the project root, not in the spec directory.

---

"""
    return spec_context + _format_task_intake_section(spec_dir) + prompt
...
def get_coding_prompt(spec_dir: Path) -> str:
    ...
    if not prompt_file.exists():
        raise FileNotFoundError(
            ...
            "Make sure the apps/backend/prompts/coder.md file exists."
        )
...
The project root is the parent of .auto-iflow. All code goes in the project root, not in the spec directory.
...
def get_followup_planner_prompt(spec_dir: Path) -> str:
    ...
    if not prompt_file.exists():
        raise FileNotFoundError(
            ...
            "Make sure the apps/backend/prompts/followup_planner.md file exists."
        )
...
- Project root: Parent of .auto-iflow (where code should be implemented)
...
"""
    ...
    spec_context += _format_task_intake_section(spec_dir)
    spec_context += "---\n\n"
...
"""
    return spec_context + _format_task_intake_section(spec_dir) + base_prompt
