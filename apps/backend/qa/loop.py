...
from init import AUTO_BUILD_BRANCH_NAMESPACE, resolve_auto_build_dir
...
async def run_qa_validation_loop(
    ...
    Args:
        ...
        model: Optional model override (falls back to model routing)
    ...
    qa_model, qa_thinking_level, qa_thinking_budget = resolve_model(
        phase="validation",
        spec_dir=spec_dir,
    ...
    debug(
        ...
        model=qa_model,
        thinking_level=qa_thinking_level,
    ...
    if has_human_feedback:
        ...
        fixer_thinking_budget = qa_thinking_budget
    ...
    while qa_iteration < MAX_QA_ITERATIONS:
        ...
        if status == "approved":
            ...
            print("\nNext steps:")
            print(f"  1. Review the {AUTO_BUILD_BRANCH_NAMESPACE}/* branch")
            ...
            if sync_plan_status_after_qa(spec_dir):
                debug(
                    "qa_loop",
                    "Synced plan status to human_review after QA approval",
                )
            else:
                debug_warning(
                    "qa_loop",
                    "Plan status sync skipped or failed after QA approval",
                )
