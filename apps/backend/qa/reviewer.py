...
from .proof_gate import validate_proof_gate
...
"""
    ...
    try:
        debug("qa_reviewer", "Sending query to agent...")
        ...
        while True:
            try:
                ...
                message = (
                    f"No agent output for {timeout_display}s; aborting QA session"
                )
                debug_error(
                    "qa_reviewer",
                ...
                if task_logger:
                    task_logger.log_error(message, LogPhase.VALIDATION)
        ...
        if status and status.get("status") == "approved":
            proof_ok, missing = validate_proof_gate(spec_dir)
            if not proof_ok:
                debug_error("qa_reviewer", "Proof gate failed", missing=missing)
                ...
                if plan is not None:
                    ...
                    plan["qa_signoff"] = {
                        ...
                        "issues_found": issues,
                        "verified_by": "qa_proof_gate",
                    }
                    save_implementation_plan(spec_dir, plan)
                return "rejected", "Proof gate failed: missing required proofs"
