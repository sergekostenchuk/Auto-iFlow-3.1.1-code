#!/usr/bin/env python3
"""
Memory Query CLI for Auto-iFlow.
...
async def _async_semantic_search(args):
    ...
    try:
        # Add auto-iflow to path for imports
        auto_iflow_dir = Path(__file__).parent
        if str(auto_iflow_dir) not in sys.path:
...
def main():
    parser = argparse.ArgumentParser(
        description="Query LadybugDB memory database for Auto-iFlow"
    )
