...
from .claude_client import IFLOW_SDK_AVAILABLE, IFlowAnalysisClient
...
class AIAnalyzerRunner:
    ...
    def __init__(self, project_dir: Path, project_index: dict[str, Any]):
        ...
        self.cache_manager = CacheManager(data_dir / "ai_cache")
    ...
    async def run_full_analysis(
        ...
        if not IFLOW_SDK_AVAILABLE:
            print("✗ iFlow SDK not available. Cannot run AI analysis.")
            return {"error": "iFlow SDK not installed"}
    ...
    async def _run_single_analyzer(self, analyzer_name: str) -> dict[str, Any]:
        ...
        # Run iFlow query
        client = IFlowAnalysisClient(self.project_dir)
