...
class IFlowBatchAnalyzer:
    """
    iFlow-based batch analyzer for GitHub issues.

    Instead of doing O(n²) pairwise comparisons, this uses a single model call
    to analyze a group of issues and suggest optimal batching.
    """

    def __init__(self, project_dir: Path | None = None):
        """Initialize iFlow batch analyzer."""
    ...
    async def analyze_and_batch_issues(
        ...
        Uses a SINGLE model call to analyze all issues and group them intelligently.
        ...
        try:
            ...
            logger.error(f"iFlow setup not available: {e}")
            # Fallback: each issue is its own batch
            return [
                {
                    "issue_numbers": [issue["number"]],
                    "theme": issue.get("title", ""),
                    "reasoning": "iFlow SDK not available",
                    "confidence": 0.5,
                }
                for issue in issues
            ]
        if importlib.util.find_spec("iflow_sdk") is None:
        ...
        if not has_iflow_auth():
            logger.warning("iFlow auth not configured, skipping batch analysis")
            return [
                {
                    "issue_numbers": [issue["number"]],
                    "theme": issue.get("title", ""),
                    "reasoning": "iFlow auth not configured",
                    "confidence": 0.5,
                }
                for issue in issues
            ]
    ...
    {{
      ...
      "confidence": 0.95
        ...
        try:
            ...
            # Use a strong general model for better analysis (still just 1 call)
            from core.simple_client import create_simple_client

            try:
                auto_build_path = resolve_auto_build_dir(self.project_dir).name
                model_id, _, _ = resolve_model(
                    feature="github",
                    role="batch",
...
class IssueBatcher:
    ...
    Usage:
        batcher = IssueBatcher(
            github_dir=Path(".auto-iflow/github"),
            repo="owner/repo",
        )
    ...
    def __init__(
        ...
        validation_thinking_budget: int | None = None,
    ):
        ...
        # Initialize iFlow batch analyzer
        self.analyzer = IFlowBatchAnalyzer(project_dir=self.project_dir)

        # Initialize batch validator (uses iFlow auth)
        if validate_batches:
            auto_build_path = resolve_auto_build_dir(self.project_dir).name
            model_id, _, resolved_budget = resolve_model(
                feature="github",
                role="batch",
                project_dir=self.project_dir,
                auto_build_path=auto_build_path,
                cli_model=validation_model,
            )
            if validation_thinking_budget is None:
                validation_thinking_budget = resolved_budget or 10000
    ...
    def _pre_group_by_labels_and_keywords(
        ...
        This dramatically reduces the number of model calls needed
    ...
    async def _analyze_issues_with_agents(
        ...
        """
        Analyze issues using AI agents to suggest batches.

        Uses a two-phase approach:
        1. Fast O(n) pre-grouping by labels and keywords (no AI calls)
        2. One model call PER PRE-GROUP to analyze and suggest sub-batches

        For 51 issues, this might result in ~5-10 model calls instead of 1275.
        ...
        logger.info(
            f"Agent-based batching: {total_api_calls_new} model calls "
            f"(was {total_api_calls_naive} with pairwise, saved {total_api_calls_naive - total_api_calls_new})"
        )

        # Phase 2: Use the model to analyze each pre-group
        ...
        for group in pre_groups:
            ...
            # Use the model to analyze this group and suggest batches
            logger.info(f"Analyzing pre-group of {len(group)} issues with batch analyzer")
        ...
        for cluster in clusters:
            ...
            for i, issue_a in enumerate(cluster):
                ...
                for issue_b in cluster[i + 1 :]:
                    ...
                    reasoning[issue_a][issue_b] = "Grouped by batch analysis"
                    reasoning[issue_b][issue_a] = "Grouped by batch analysis"
