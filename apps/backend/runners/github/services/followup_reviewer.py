...
- AI-powered review (thorough, uses iFlow)
"""
...
class FollowupReviewer:
    ...
    def _generate_followup_summary(
        ...
        lines.append("---")
        lines.append("_Generated by Auto iFlow Follow-up Review_")
    ...
    async def _run_ai_review(
        ...
        Uses the iFlow SDK compatibility layer's structured output support to guarantee
...
"""
        ...
        try:
            # Use iFlow SDK compatibility layer query() with structured outputs
            from iflow_agent_sdk import IFlowAgentOptions, iflow_query
            from init import resolve_auto_build_dir
            from phase_config import resolve_model

            model_override = (self.config.model or "").strip()
            if model_override.lower() in ("", "auto", "default", "profile"):
                model_override = None
            thinking_override = (self.config.thinking_level or "").strip()
            if thinking_override.lower() in ("", "auto", "default", "profile"):
                thinking_override = None

            auto_build_path = resolve_auto_build_dir(self.project_dir).name
            model, thinking_level, thinking_budget = resolve_model(
                feature="github",
                role="followUp",
                project_dir=self.project_dir,
                auto_build_path=auto_build_path,
                cli_model=model_override,
                cli_thinking=thinking_override,
            )
            ...
            async for message in iflow_query(
                prompt=user_message,
                options=IFlowAgentOptions(
                    model=model,
                ...
                if msg_type == "ResultMessage":
                    ...
                    if subtype == "error_max_structured_output_retries":
                        logger.warning(
                            "iFlow SDK could not produce valid structured output after retries"
                        )
                        return None
