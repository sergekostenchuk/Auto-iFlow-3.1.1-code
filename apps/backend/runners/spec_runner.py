...
if sys.version_info < (3, 10):  # noqa: UP036
    sys.exit(
        f"Error: Auto-iFlow requires Python 3.10 or higher.\n"
...
# Add backend to path (parent of runners/)
...
Examples:
  ...
  python spec_runner.py --interactive
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Override model for spec phases (default: use model routing settings)",
    )
    parser.add_argument(
        "--thinking-level",
        type=str,
        ...
        help="Override thinking level for spec phases (default: use model routing settings)",
    )
    parser.add_argument(
        "--no-ai-assessment",
        action="store_true",
    ...
    # Auto-detect if running from within the source directory
    if project_dir.name in ("auto-iflow",) and (project_dir / "run.py").exists():
        # Running from within source directory, go up 1 level
        project_dir = project_dir.parent
    elif not (project_dir / ".auto-iflow").exists():
        # No data directory found - try to find project root
        for parent in project_dir.parents:
            if (parent / ".auto-iflow").exists():
                project_dir = parent
                break
    ...
    debug(
        ...
        thinking_level=orchestrator.thinking_level,
    )

    try:
        try:
            write_resolved_model_snapshot(orchestrator.spec_dir)
        except Exception as exc:
            debug_error("spec_runner", "Failed to write resolvedModel snapshot", error=str(exc))
        ...
        if not args.no_build:
            ...
            if not review_state.is_approved():
                ...
                print(
                    f"  {highlight(f'python review.py --spec-dir {orchestrator.spec_dir}')}"
                )
                print()
                print(
                ...
                example_cmd = (
                    'python spec_runner.py --task "..." --auto-approve'
                )
        ...
        print(
            f"To continue: python spec_runner.py --continue {orchestrator.spec_dir.name}"
        )
        sys.exit(1)
