...
_cached_profile_mtime: float | None = None  # Track profile mtime
_cached_allowlist_mtime: float | None = None  # Track allowlist mtime


def _get_file_mtime(path: Path) -> float | None:
    """Get the modification time of a file, or None if not exists."""
    try:
...
def _get_allowlist_path(project_dir: Path) -> Path:
    """Get the allowlist path for a project."""
    return project_dir / ALLOWLIST_FILENAME


def _get_profile_mtime(project_dir: Path) -> float | None:
    """Get modification time for the profile file."""
    return _get_file_mtime(_get_profile_path(project_dir))


def _get_allowlist_mtime(project_dir: Path) -> float | None:
    """Get modification time for the allowlist file."""
    return _get_file_mtime(_get_allowlist_path(project_dir))
