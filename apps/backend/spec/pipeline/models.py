...
from init import init_auto_build_dir, resolve_auto_build_dir
...
def get_specs_dir(project_dir: Path, ensure_write: bool = False) -> Path:
    """Get the specs directory path.

    IMPORTANT: .auto-iflow is the default data directory for project specs.

    This function also ensures the data directory is added to .gitignore on first use.

    Args:
        project_dir: The project root directory

    Returns:
        Path to the specs directory within the data directory
    """
    if ensure_write:
        # Initialize data directory and ensure it's in .gitignore
...
PHASE_DISPLAY: dict[str, tuple[str, str]] = {
    ...
    "env_reality_check": ("ENV REALITY CHECK", Icons.SHIELD),
    "preflight": ("SCOPE PREFLIGHT", Icons.SHIELD),
    "senior_review": ("SENIOR REVIEW", Icons.SHIELD),
