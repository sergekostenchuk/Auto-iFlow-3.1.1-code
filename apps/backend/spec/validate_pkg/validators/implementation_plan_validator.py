...
def _matches_test_plan(command: str, test_plan: list[str]) -> bool:
    if not test_plan:
        return False
    ...
    for entry in test_plan:
        entry_norm = _normalize_command(entry)
        if not entry_norm:
            continue
        if command_norm == entry_norm:
            return True
        if command_norm.startswith(f"{entry_norm} "):
            return True
        if entry_norm.startswith(f"{command_norm} "):
            return True
    return False
...
class ImplementationPlanValidator:
    ...
    def __init__(self, spec_dir: Path):
        ...
        self._test_plan_cache: list[str] | None = None
    ...
    def validate(self) -> ValidationResult:
        ...
        if not plan_file.exists():
            ...
            fixes.append(
                f"Run: python apps/backend/planner_lib/main.py --spec-dir {self.spec_dir}"
            )
        ...
        try:
            ...
            fixes.append(
                "Regenerate with: python apps/backend/planner_lib/main.py --spec-dir "
                + str(self.spec_dir)
            )
            return ValidationResult(False, "plan", errors, warnings, fixes)
    ...
    def _load_test_plan(self) -> list[str]:
        if self._test_plan_cache is not None:
            return self._test_plan_cache
        scope_file = self.spec_dir / "scope_contract.json"
        if not scope_file.exists():
            self._test_plan_cache = []
            return []
        try:
    ...
    def _validate_subtask(
        ...
        if "verification" in subtask:
            ...
            if "type" not in ver:
                ...
                run_cmd = ver.get("run") or ver.get("command")
                if ver.get("type") in {"command", "e2e"} and isinstance(run_cmd, str):
                    test_plan = self._load_test_plan()
                    if _matches_test_plan(run_cmd, test_plan):
                        self._warnings.append(
                            "Phase "
                            f"{phase_idx + 1}, Subtask {subtask_idx + 1}: "
                            "verification command matches test_plan. "
                            "Tests must run only in Post-Code stage."
                        )
