...
ccstatusline Configuration:
    Add to ~/.config/ccstatusline/settings.json:
    {
        "widgets": [
            {
                "type": "custom_command",
                "command": "python /path/to/auto-iflow/statusline.py",
                "refresh": 5000
            }
        ]
    }
...
def find_project_root() -> Path:
    """Find the project root by looking for .auto-iflow status files."""
    cwd = Path.cwd()

    # Check current directory - prioritize .auto-iflow (installed instance)
    if (cwd / ".auto-iflow").exists():
        return cwd
    if (cwd / ".auto-iflow-status").exists():
        return cwd
    ...
    for parent in cwd.parents:
        if (parent / ".auto-iflow").exists():
            return parent
        if (parent / ".auto-iflow-status").exists():
            return parent
