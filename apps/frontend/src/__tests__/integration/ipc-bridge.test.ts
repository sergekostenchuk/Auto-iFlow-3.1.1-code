...
describe('IPC Bridge Integration', () => {
  ...
  describe('Preload script API', () => {
    ...
    describe('Task operations', () => {
      ...
      it('should have createSandboxTask method', async () => {
        const createSandboxTask = electronAPI['createSandboxTask'] as (
          title: string,
          desc: string,
          metadata?: unknown,
          sourceProjectId?: string
        ) => Promise<unknown>;
        await createSandboxTask('Sandbox Task', 'Sandbox description');

        // Third argument is optional metadata (undefined when not provided)
        // Fourth argument is optional source project id (undefined when not provided)
        expect(mockIpcRenderer.invoke).toHaveBeenCalledWith(
          'task:createSandbox',
          'Sandbox Task',
          'Sandbox description',
    ...
    describe('Dialog operations', () => {
      ...
      it('should have selectFile method', async () => {
        const selectFile = electronAPI['selectFile'] as () => Promise<unknown>;
        await selectFile();

        expect(mockIpcRenderer.invoke).toHaveBeenCalledWith('dialog:selectFile');
      });
    });
  ...
  describe('IPC channel constants', () => {
    it('should use consistent channel names', async () => {
      ...
      expect(IPC_CHANNELS.DIALOG_SELECT_FILE).toBe('dialog:selectFile');
