...
// Auto-iFlow source path (for getAutoBuildSourcePath to find)
const AUTO_IFLOW_SOURCE = path.join(TEST_DIR, 'auto-iflow-source');

// Setup test directories
function setupTestDirs(): void {
  mkdirSync(TEST_PROJECT_PATH, { recursive: true });

  // Create auto-iFlow source directory that getAutoBuildSourcePath looks for
  mkdirSync(AUTO_IFLOW_SOURCE, { recursive: true });

  // Create runners subdirectory with spec_runner.py marker (used by getAutoBuildSourcePath)
  mkdirSync(path.join(AUTO_IFLOW_SOURCE, 'runners'), { recursive: true });

  // Create mock spec_runner.py in runners/ subdirectory (used as backend marker)
  writeFileSync(
    path.join(AUTO_IFLOW_SOURCE, 'runners', 'spec_runner.py'),
    '# Mock spec runner\nprint("Starting spec creation")'
  );
  // Create mock run.py
  writeFileSync(
    path.join(AUTO_IFLOW_SOURCE, 'run.py'),
...
describe('Subprocess Spawn Integration', () => {
  ...
  describe('AgentManager', () => {
    it('should spawn Python process for spec creation', async () => {
      ...
      manager.configure(undefined, AUTO_IFLOW_SOURCE);
      ...
      expect(spawn).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          cwd: AUTO_IFLOW_SOURCE,  // Process runs from auto-iFlow source directory
    ...
    it('should use configured Python path', async () => {
      ...
      const manager = new AgentManager();
      manager.configure('/custom/python3', AUTO_IFLOW_SOURCE);
