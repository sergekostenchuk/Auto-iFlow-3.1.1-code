...
describe('ProjectStore', () => {
  ...
  describe('addProject', () => {
    ...
    it('should detect auto-iflow directory if present', async () => {
      // Create .auto-iflow directory (the data directory, not source code)
      mkdirSync(path.join(TEST_PROJECT_PATH, '.auto-iflow'), { recursive: true });
      ...
      expect(project.autoBuildPath).toBe('.auto-iflow');
    });
  ...
  describe('getTasks', () => {
    ...
    it('should read tasks from filesystem correctly', async () => {
      // Create spec directory structure in .auto-iflow (the data directory)
      const specsDir = path.join(TEST_PROJECT_PATH, '.auto-iflow', 'specs', '001-test-feature');
    ...
    it('should determine status as backlog when no subtasks completed', async () => {
      const specsDir = path.join(TEST_PROJECT_PATH, '.auto-iflow', 'specs', '002-pending');
      mkdirSync(specsDir, { recursive: true });
    ...
    it('should determine status as ai_review when all subtasks completed', async () => {
      const specsDir = path.join(TEST_PROJECT_PATH, '.auto-iflow', 'specs', '003-complete');
      mkdirSync(specsDir, { recursive: true });
    ...
    it('should determine status as human_review when QA report rejected', async () => {
      const specsDir = path.join(TEST_PROJECT_PATH, '.auto-iflow', 'specs', '004-rejected');
      mkdirSync(specsDir, { recursive: true });
    ...
    it('should determine status as human_review when QA report approved', async () => {
      ...
      const specsDir = path.join(TEST_PROJECT_PATH, '.auto-iflow', 'specs', '005-approved');
      mkdirSync(specsDir, { recursive: true });
    ...
    it('should determine status as done when plan status is explicitly done', async () => {
      ...
      const specsDir = path.join(TEST_PROJECT_PATH, '.auto-iflow', 'specs', '006-done');
      mkdirSync(specsDir, { recursive: true });
