...
export class AgentProcessManager {
  ...
  /**
   * Get the auto-iflow source path (detects automatically if not configured)
   */
  ...
  /**
   * Load environment variables from the project's data directory .env file
  ...
  /**
   * Load environment variables from the backend source .env file
   */
  ...
  async spawnProcess(
    ...
    childProcess.on('exit', (code: number | null) => {
      ...
      if (code !== 0) {
        ...
        const stdoutSummary = stdoutBuffer.trim();
        const fallbackSummary = allOutput.trim();
        const summary = stderrSummary || stdoutSummary || fallbackSummary || `Process exited with code ${code}`;
        const truncated = summary.length > 4000 ? summary.slice(-4000) : summary;
        this.emitter.emit('error', taskId, truncated);
      }
   ...
   * 3. Project data .env (.auto-iflow or legacy data dir) - Frontend-configured settings
