...
export class AgentQueueManager {
  ...
  private async spawnIdeationProcess(
    ...
    // Run from backend source directory so imports work correctly
    const autoBuildSource = this.processManager.getAutoBuildSourcePath();
    const cwd = autoBuildSource || process.cwd();
    const autoBuildPath = getAutoBuildPath(projectPath) || DEFAULT_AUTO_BUILD_PATH;
    ...
    // 3. combinedEnv (.auto-iflow/.env for CLI usage)
    ...
    const tokenSource = profileEnv['CLAUDE_CODE_OAUTH_TOKEN']
      ...
      : (combinedEnv['CLAUDE_CODE_OAUTH_TOKEN'] ? `${autoBuildPath}/.env` : 'not found');
    ...
    childProcess.stdout?.on('data', (data: Buffer) => {
      ...
      if (typeCompleteMatch) {
        ...
        const typeFilePath = path.join(projectPath, autoBuildPath, 'ideation', `${ideationType}_ideas.json`);
    ...
    childProcess.on('exit', (code: number | null) => {
      ...
      if (code === 0) {
        ...
        if (storedProjectPath) {
          try {
            const ideationFilePath = path.join(storedProjectPath, autoBuildPath, 'ideation', 'ideation.json');
  ...
  private async spawnRoadmapProcess(
    ...
    // Run from auto-iflow source directory so imports work correctly
    const autoBuildSource = this.processManager.getAutoBuildSourcePath();
    const cwd = autoBuildSource || process.cwd();
    const autoBuildPath = getAutoBuildPath(projectPath) || DEFAULT_AUTO_BUILD_PATH;
    ...
    childProcess.on('exit', (code: number | null) => {
      ...
      if (code === 0) {
        ...
        if (storedProjectPath) {
          try {
            const roadmapAutoBuildPath = getAutoBuildPath(storedProjectPath) || DEFAULT_AUTO_BUILD_PATH;
            const roadmapFilePath = path.join(storedProjectPath, roadmapAutoBuildPath, 'roadmap', 'roadmap.json');
