...
function cleanupStaleUpdateMetadata(): void {
  ...
  const stalePaths = [
    join(userData, 'auto-iflow-source'),
...
function getIconPath(): string {
  ...
  if (process.platform === 'darwin') {
    ...
    iconName = is.dev ? 'auto-iflow-256.png' : 'auto-iflow.icns';
  } else if (process.platform === 'win32') {
    iconName = 'auto-iflow.ico';
  } else {
    iconName = 'auto-iflow.png';
  }
...
function createWindow(): void {
  ...
  mainWindow.webContents.on('console-message', (_event, level, message, line, sourceId) => {
    const levelMap = ['log', 'warn', 'error', 'debug'];
    const levelLabel = levelMap[level] ?? 'log';
    const formatted = `[renderer:${levelLabel}] ${message} (${sourceId}:${line})`;
    if (levelLabel === 'error') {
      appLog.error(formatted);
    } else if (levelLabel === 'warn') {
      appLog.warn(formatted);
    } else {
      appLog.info(formatted);
    }
  });
  ...
  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    ...
    appLog.error(`[renderer:load-failed] ${errorCode} ${errorDescription} (${validatedURL})`);
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    appLog.error(`[renderer:crash] reason=${details.reason} exitCode=${details.exitCode}`);
  });
    ...
    const template = [
      {
        role: 'cut',
        label: 'Cut',
        enabled: params.isEditable && (params.editFlags?.canCut ?? false)
      },
      {
        role: 'copy',
        label: 'Copy',
        enabled: (params.editFlags?.canCopy ?? false) || Boolean(params.selectionText)
      },
      {
        role: 'paste',
        label: 'Paste',
        enabled: params.isEditable && (params.editFlags?.canPaste ?? false)
      },
      { type: 'separator' },
      {
        role: 'selectAll',
  ...
  // Open DevTools in development (disabled by default - use F12 to toggle)
  // if (is.dev) {
...
app.whenReady().then(() => {
  ...
  electronApp.setAppUserModelId('com.autoiflow.ui');
  ...
  // Load settings and configure agent manager with Python and auto-build paths
