...
This directory contains the refactored IPC (Inter-Process Communication) handlers for Auto-iFlow UI, organized into domain-specific modules for better maintainability and code organization.
...
- `PROJECT_INITIALIZE` - Initialize .auto-iflow directory
- `PROJECT_CHECK_VERSION` - Check initialization status
- `project:has-local-source` - Check if project has local auto-iflow source
