...
This directory contains the refactored context-related IPC handlers for the Auto-iFlow UI application. The handlers manage project context, memory systems (both file-based and Graphiti/LadybugDB), and project index operations.
...
test('buildMemoryStatus returns correct status', () => {
  const status = buildMemoryStatus('/path/to/project', '.auto-iflow');
...
- [Project Memory System](../../../../apps/backend/integrations/graphiti/memory.py)
- [Graphiti Memory Integration](../../../../apps/backend/integrations/graphiti/config.py)
