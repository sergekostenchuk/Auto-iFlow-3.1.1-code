...
export function getGraphitiDatabaseDetails(projectEnvVars: EnvironmentVars): GraphitiDatabaseDetails {
  const dbPathOverride = projectEnvVars['GRAPHITI_DB_PATH'] || process.env.GRAPHITI_DB_PATH;
  const databaseOverride = projectEnvVars['GRAPHITI_DATABASE'] || process.env.GRAPHITI_DATABASE;

  return resolveGraphitiStorageDefaults(dbPathOverride, databaseOverride);
}
