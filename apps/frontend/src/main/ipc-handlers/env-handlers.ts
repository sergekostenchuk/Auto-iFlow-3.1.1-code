...
function getProjectEnvJsonPath(projectPath: string, autoBuildPath: string): string {
  return path.join(projectPath, autoBuildPath, 'project.env.json');
}

function readProjectEnvJson(projectPath: string, autoBuildPath: string): Record<string, unknown> {
  const envJsonPath = getProjectEnvJsonPath(projectPath, autoBuildPath);
  ...
  try {
    ...
    debugError('[ENV_GET] Failed to parse project.env.json:', error);
    return {};
  }
}

function writeProjectEnvJson(projectPath: string, autoBuildPath: string, data: Record<string, unknown>): void {
  const envJsonPath = getProjectEnvJsonPath(projectPath, autoBuildPath);
  writeFileSync(envJsonPath, JSON.stringify(data, null, 2));
}
...
export function registerEnvHandlers(
  ...
  const generateEnvContent = (
    ...
    const content = `# Auto-iFlow Framework Environment Variables
...
# If not set, Auto-iFlow will auto-detect main/master, or fall back to current branch
...
${existingVars['GRAPHITI_DATABASE'] ? `GRAPHITI_DATABASE=${existingVars['GRAPHITI_DATABASE']}` : '# GRAPHITI_DATABASE=auto_iflow_memory'}
${existingVars['GRAPHITI_DB_PATH'] ? `GRAPHITI_DB_PATH=${existingVars['GRAPHITI_DB_PATH']}` : '# GRAPHITI_DB_PATH=~/.auto-iflow/graphs'}
`;
  ...
  ipcMain.handle(
    ...
    async (_, projectId: string): Promise<IPCResult<ProjectEnvConfig>> => {
      ...
      const envJson = readProjectEnvJson(project.path, project.autoBuildPath);
      if (envJson.modelRouting) {
        config.modelRouting = envJson.modelRouting as ProjectEnvConfig['modelRouting'];
      }
    ...
    async (_, projectId: string, config: Partial<ProjectEnvConfig>): Promise<IPCResult> => {
      ...
      try {
        ...
        if (config.modelRouting !== undefined) {
          const existingEnvJson = readProjectEnvJson(project.path, project.autoBuildPath);
          const updatedEnvJson = {
            ...existingEnvJson,
            modelRouting: config.modelRouting,
          };
          writeProjectEnvJson(project.path, project.autoBuildPath, updatedEnvJson);
        }
