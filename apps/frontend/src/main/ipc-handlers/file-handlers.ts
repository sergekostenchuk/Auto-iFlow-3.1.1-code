...
export function registerFileHandlers(): void {
  ...
  ipcMain.handle(
    IPC_CHANNELS.FILE_EXPLORER_OPEN,
    async (_, targetPath: string): Promise<IPCResult<void>> => {
      try {
        ...
        const stats = statSync(validation.path);
        const dirPath = stats.isDirectory() ? validation.path : path.dirname(validation.path);
