...
import fs from 'fs';
import { IPC_CHANNELS, MODEL_ID_MAP, DEFAULT_FEATURE_MODELS, DEFAULT_FEATURE_THINKING, DEFAULT_AUTO_BUILD_PATH } from '../../../shared/constants';
import { getGitHubConfig, githubFetch } from './utils';
import { readSettingsFile } from '../../settings-utils';
import { getAugmentedEnv } from '../../env-utils';
import { getMemoryService } from '../../memory-service';
import { resolveGraphitiStorageDefaults } from '../../config-paths';
...
async function savePRReviewToMemory(
  ...
  try {
    const defaults = resolveGraphitiStorageDefaults();
    const memoryService = getMemoryService(defaults);
...
function getGitHubDir(project: Project): string {
  const autoBuildPath = project.autoBuildPath || DEFAULT_AUTO_BUILD_PATH;
  return path.join(project.path, autoBuildPath, 'github');
}
      ...
      const postResult = await withProjectOrNull(projectId, async (project) => {
        ...
        try {
          ...
          if (options?.forceApprove) {
            ...
            body = `## ✅ Auto-iFlow Review - APPROVED\n\n`;
            ...
            body += `*Generated by Auto-iFlow*`;
          } else {
            // Standard review format
            body = `## 🤖 Auto-iFlow PR Review\n\n${result.summary}\n\n`;
            ...
            body += `---\n*This review was generated by Auto-iFlow.*`;
          }
