...
/**
 * Get the auto-iflow backend path for a project
 *
...
export function getBackendPath(project: Project): string | null {
  ...
  const isFrontendCwd =
    cwdParts.length >= 2 &&
    cwdParts[cwdParts.length - 2] === 'apps' &&
    cwdParts[cwdParts.length - 1] === 'frontend';
  ...
  const possiblePaths = [
    ...
    // If running from apps/frontend in dev/test
    isFrontendCwd ? path.resolve(cwd, '..', 'backend') : null,
