 ...
 * Pre-curated models suitable for Auto-iFlow memory system
 */
  ...
  ipcMain.handle(
    ...
    async (_, dbPath?: string, database?: string): Promise<IPCResult<GraphitiValidationResult>> => {
      try {
        ...
        const defaults = resolveGraphitiStorageDefaults(dbPath, database);
        const service = getMemoryService(defaults);
    ...
    async (
      ...
      try {
        ...
        if (!isKuzuAvailable()) {
          ...
          const defaults = resolveGraphitiStorageDefaults(config.dbPath, config.database);
          const service = getMemoryService(defaults);
