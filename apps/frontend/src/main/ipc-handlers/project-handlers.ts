...
import { IPC_CHANNELS, DEFAULT_AUTO_BUILD_PATH } from '../../shared/constants';
import type {
  Project,
  ProjectSettings,
  IPCResult,
...
export function registerProjectHandlers(
  ...
  ipcMain.handle(
    ...
    async (): Promise<IPCResult<Project[]>> => {
      // Validate that data directories still exist for all projects
      ...
      if (resetIds.length > 0) {
        console.warn('[IPC] PROJECT_LIST: Detected missing data directories for', resetIds.length, 'project(s)');
      }
    ...
    async (_, projectId: string): Promise<IPCResult<InitializationResult>> => {
      try {
        ...
        if (result.success) {
          ...
          projectStore.updateAutoBuildPath(projectId, DEFAULT_AUTO_BUILD_PATH);
        }
  ...
  // Version tracking for data directory is removed since it only contains data
  ipcMain.handle(
      ...
      try {
        ...
        return {
          ...
          data: {
            ...
            updateAvailable: false // No updates for data directories - they contain only project data
          }
        };
      } catch (error) {
        return {
  ...
  // Check if project has local backend source (is dev project)
  ipcMain.handle(
