...
import type { CustomModel } from '../../shared/types/profile';
...
/**
 * Auto-detect the auto-iflow source path relative to the app location.
...
const detectAutoBuildSourcePath = (): string | null => {
  ...
  for (const p of possiblePaths) {
    ...
    // This prevents matching unrelated directories that don't have the runners
  ...
  console.warn('[detectAutoBuildSourcePath] Could not auto-detect Auto-iFlow source path. Please configure manually in settings.');
  console.warn('[detectAutoBuildSourcePath] Set DEBUG=1 environment variable for detailed path checking.');
  return null;
};

const resolveModelsRegistryPath = (): string | null => {
  const candidates: string[] = [];

  if (is.dev) {
    candidates.push(
      path.resolve(__dirname, '..', '..', '..', '..', 'shared', 'models.json'),
      path.resolve(process.cwd(), 'shared', 'models.json')
    );
  } else {
    candidates.push(
      path.resolve(process.resourcesPath, 'shared', 'models.json'),
      path.resolve(app.getAppPath(), '..', 'shared', 'models.json'),
      path.resolve(app.getAppPath(), 'shared', 'models.json')
    );
  }
...
const mapCustomModel = (model: CustomModel): ModelRegistryEntry | null => {
...
const loadModelRegistry = async (): Promise<ModelRegistryData> => {
  const registryPath = resolveModelsRegistryPath();
  if (!registryPath) {
    throw new Error('models.json not found in shared resources');
  }
  ...
  if (!Array.isArray(data.models)) {
    throw new Error('models.json missing models list');
  }

  let mergedModels = data.models as ModelRegistryEntry[];
  try {
    ...
    if (customModels.length > 0) {
      ...
      mergedModels.forEach((model) => merged.set(model.id, model));
      customModels
        .map(mapCustomModel)
        .filter((model): model is ModelRegistryEntry => Boolean(model))
        .forEach((model) => merged.set(model.id, model));
      mergedModels = Array.from(merged.values());
    }
  } catch (error) {
    console.warn('[loadModelRegistry] Failed to merge custom models:', error);
  }

  return {
    models: mergedModels,
    legacyAliases: data.legacyAliases ?? {},
    bootstrapModel: data.bootstrapModel
  };
};
...
export function registerSettingsHandlers(
  ...
  ipcMain.handle(
    ...
    async (): Promise<IPCResult<AppSettings>> => {
      ...
      // See: https://github.com/AndyMik90/Auto-iFlow/issues/XXX
      ...
      const sandboxOverride = parseBooleanEnv(process.env.AUTO_IFLOW_SANDBOX_TASKS);
      if (sandboxOverride !== undefined) {
        settings.sandboxTasksEnabled = sandboxOverride;
      }
  ...
  ipcMain.handle(
    IPC_CHANNELS.MODELS_GET_REGISTRY,
    async (): Promise<IPCResult<ModelRegistryData>> => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_, config: { claudeOAuthToken?: string }): Promise<IPCResult> => {
      try {
        ...
        // Generate content
        const lines: string[] = [
          '# Auto-iFlow Framework Environment Variables',
          '# Managed by Auto-iFlow UI',
          '',
