...
- `../../app-updater` - Source paths
- `../../rate-limit-detector` - Profile environment

## Architecture Notes

### Worktree Architecture
Each task spec has its own isolated worktree at `.auto-iflow/worktrees/tasks/{spec-name}/` (legacy `.worktrees/{spec-name}/`):
- Enables safe parallel development
- Each spec has dedicated branch: `auto-iflow/{spec-name}`
