...
import { hasIflowAuth } from '../../iflow-auth';
import {
  getPlanPath,
  persistPlanStatus,
...
import { pausePlanImportQueueForTask } from './plan-import-handlers';
...
export function registerTaskExecutionHandlers(
    ...
    (_, taskId: string, _options?: TaskStartOptions) => {
      ...
      if (!gitStatus.isGitRepo) {
        ...
        mainWindow.webContents.send(
          IPC_CHANNELS.TASK_ERROR,
          taskId,
          'Git repository required. Please run "git init" in your project directory. Auto-iFlow uses git worktrees for isolated builds.'
        );
        return;
      }
      ...
      // Check authentication - iFlow requires valid auth to run tasks
      if (!hasIflowAuth()) {
        console.warn('[TASK_START] No iFlow authentication configured');
        mainWindow.webContents.send(
          IPC_CHANNELS.TASK_ERROR,
          taskId,
          'iFlow authentication required. Configure an API key or Web login in Settings > Integrations.'
        );
        ...
        appendTaskLogEntry(
          ...
          task.specId,
          {
            ...
            type: 'error',
            content: 'iFlow authentication required. Configure an API key or Web login in Settings > Integrations.',
            phase: 'planning',
            detail: 'Task start blocked: missing iFlow auth',
            collapsed: false
          },
          'planning'
        );
  ...
  ipcMain.on(IPC_CHANNELS.TASK_STOP, (_, taskId: string) => {
    ...
    const { task, project } = findTaskAndProject(taskId);
    const isImportController = task?.metadata?.planImportId && task.metadata.planImportId === taskId;
    const pausedQueue = pausePlanImportQueueForTask(taskId);
    const shouldPauseImport = pausedQueue.paused && pausedQueue.importTaskId;
    ...
    if (mainWindow) {
      mainWindow.webContents.send(
        IPC_CHANNELS.TASK_STATUS_CHANGE,
        taskId,
        shouldPauseImport && isImportController ? 'paused' : 'backlog'
      );
      if (shouldPauseImport && !isImportController) {
        mainWindow.webContents.send(
          IPC_CHANNELS.TASK_STATUS_CHANGE,
          pausedQueue.importTaskId,
          'paused'
        );
      }
    }
    ...
    if (task && project) {
      ...
      setImmediate(async () => {
        const persistStart = Date.now();
        try {
          const statusToPersist = shouldPauseImport && isImportController ? 'paused' : 'backlog';
          const persisted = await persistPlanStatus(planPath, statusToPersist, project.id);
          if (persisted) {
            console.warn(`[TASK_STOP] Updated plan status to ${statusToPersist}`);
          }
          if (shouldPauseImport && pausedQueue.importTaskId && pausedQueue.importSpecDir) {
            const importPlanPath = path.join(pausedQueue.importSpecDir, AUTO_BUILD_PATHS.IMPLEMENTATION_PLAN);
            const persistedImport = await persistPlanStatus(importPlanPath, 'paused', project.id);
            if (persistedImport) {
              console.warn('[TASK_STOP] Updated import controller plan status to paused');
            }
          }
          if (DEBUG) {
            const delay = persistStart - ipcSentAt;
    ...
    async (
      ...
      if (approved) {
        ...
        if (hasWorktree) {
          ...
          // IMPORTANT: Exclude data directories to preserve specs and worktree data
          const cleanResult = spawnSync(
            'git',
            ['clean', '-fd', '-e', DEFAULT_AUTO_BUILD_PATH],
            {
            cwd: project.path,
            encoding: 'utf-8',
            stdio: 'pipe'
          ...
          if (cleanResult.status === 0) {
            console.log('[TASK_REVIEW] Cleaned untracked files in main (excluding data directories)');
          }
    ...
    async (
      ...
      try {
        ...
        if (status === 'in_progress' && !agentManager.isRunning(taskId)) {
          ...
          if (!hasIflowAuth()) {
            console.warn('[TASK_UPDATE_STATUS] No iFlow authentication configured');
            if (mainWindow) {
              mainWindow.webContents.send(
                IPC_CHANNELS.TASK_ERROR,
                taskId,
            ...
            return { success: false, error: 'iFlow authentication required' };
          }
    ...
    async (
      ...
      try {
        ...
        if (autoRestart && project) {
          ...
          if (!hasIflowAuth()) {
            console.warn('[Recovery] iFlow auth check failed, cannot auto-restart task');
            ...
            return {
              ...
              data: {
                ...
                recovered: true,
                newStatus,
                message: 'Task recovered but cannot restart: iFlow authentication required. Configure an API key or Web login in Settings > Integrations.',
                autoRestarted: false
              }
            };
          }
