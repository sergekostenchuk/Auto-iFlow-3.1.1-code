...
import { IPC_CHANNELS, AUTO_BUILD_PATHS, DEFAULT_APP_SETTINGS, DEFAULT_FEATURE_MODELS, DEFAULT_FEATURE_THINKING, MODEL_ID_MAP, THINKING_BUDGET_MAP, DEFAULT_AUTO_BUILD_PATH, AUTO_BUILD_BRANCH_NAMESPACE } from '../../../shared/constants';
import type { IPCResult, WorktreeStatus, WorktreeDiff, WorktreeDiffFile, WorktreeMergeResult, WorktreeFixAction, WorktreeFixResult, WorktreeDiscardResult, WorktreeListResult, WorktreeListItem, SupportedIDE, SupportedTerminal, AppSettings } from '../../../shared/types';
...
const getTaskBranchCandidates = (specId: string) => ([
  `${AUTO_BUILD_BRANCH_NAMESPACE}/${specId}`
]);

const resolveTaskBranch = (projectPath: string, specId: string) => {
  const git = getToolPath('git');
  for (const candidate of getTaskBranchCandidates(specId)) {
    try {
      ...
      // Branch does not exist, keep checking.
    }
  }
  return `${AUTO_BUILD_BRANCH_NAMESPACE}/${specId}`;
};

const deleteTaskBranches = (projectPath: string, specId: string, debug: (...args: any[]) => void) => {
  const git = getToolPath('git');
  for (const candidate of getTaskBranchCandidates(specId)) {
    try {
      execFileSync(git, ['branch', '-D', candidate], {
        cwd: projectPath,
        encoding: 'utf-8',
        stdio: ['pipe', 'pipe', 'pipe']
      });
      debug('Task branch deleted:', candidate);
    } catch {
...
const SANDBOX_GITIGNORE_COMMIT_MESSAGE = 'chore: update gitignore';
const SANDBOX_AUTO_COMMIT_MESSAGE = 'chore(sandbox): commit task results';
const SANDBOX_STATUS_CLEANUP_MESSAGE = 'chore(sandbox): cleanup status files';
const SANDBOX_STATUS_FILES = ['.auto-iflow-status'];
const SANDBOX_CLEANUP_ALLOWLIST = new Set(['.gitignore', ...SANDBOX_STATUS_FILES]);
...
function getUncommittedFiles(projectPath: string): string[] {
  try {
    const statusOutput = execFileSync(getToolPath('git'), ['status', '--porcelain'], {
      cwd: projectPath,
      encoding: 'utf-8'
    });
    return parseGitStatusFiles(statusOutput);
  } catch {
    return [];
  }
}

function ensureGitignoreEntries(projectPath: string, entries: string[]): boolean {
  ...
  if (entriesToAdd.length === 0) return false;
...
function isTrackedFile(projectPath: string, filePath: string): boolean {
  try {
    execFileSync(getToolPath('git'), ['ls-files', '--error-unmatch', filePath], {
      cwd: projectPath,
      encoding: 'utf-8',
      stdio: ['pipe', 'pipe', 'pipe']
    });
...
function autoCommitSandboxGitignoreIfNeeded(projectPath: string, isSandbox: boolean): boolean {
  if (!isSandbox || !isGitWorkTree(projectPath)) return false;

  try {
    let gitignoreUpdated = false;
    let removedStatusFiles = false;
    const git = getToolPath('git');

    try {
      gitignoreUpdated = ensureGitignoreEntries(projectPath, SANDBOX_STATUS_FILES);
    } catch (error) {
      console.warn('[Sandbox Fix] Failed to ensure sandbox gitignore entries:', error);
    }

    for (const statusFile of SANDBOX_STATUS_FILES) {
      if (isTrackedFile(projectPath, statusFile)) {
        execFileSync(git, ['rm', '--cached', '--ignore-unmatch', statusFile], {
          cwd: projectPath,
          encoding: 'utf-8',
          stdio: ['pipe', 'pipe', 'pipe']
        });
        removedStatusFiles = true;
      }
    }

    const uncommittedFiles = getUncommittedFiles(projectPath);
    if (uncommittedFiles.length === 0 && !gitignoreUpdated && !removedStatusFiles) return false;

    const hasOnlyCleanupFiles = uncommittedFiles.every((file) => SANDBOX_CLEANUP_ALLOWLIST.has(file));
    if (!hasOnlyCleanupFiles) return false;
    ...
    const stagedStat = execFileSync(git, ['diff', '--staged', '--stat'], {
      cwd: projectPath,
      encoding: 'utf-8',
      stdio: ['pipe', 'pipe', 'pipe']
    }).trim();

    if (!stagedStat) return false;

    const hasGitignoreChange = uncommittedFiles.includes('.gitignore') || gitignoreUpdated;
    const commitMessage = hasGitignoreChange ? SANDBOX_GITIGNORE_COMMIT_MESSAGE : SANDBOX_STATUS_CLEANUP_MESSAGE;
    ...
    console.warn('[Sandbox Fix] Failed to auto-commit sandbox cleanup:', error);
    return false;
  }
}

function autoCommitSandboxResults(
  projectPath: string,
  specId: string,
  suggestedCommitMessage: string | undefined,
  debug: (...args: unknown[]) => void
): { committed: boolean; commitMessage?: string; error?: string } {
  if (!isGitWorkTree(projectPath)) return { committed: false };
  ...
  try {
    ...
    debug('Failed to ensure sandbox gitignore entries:', error);
  }

  const uncommittedFiles = getUncommittedFiles(projectPath);
  if (uncommittedFiles.length === 0 && !gitignoreUpdated) {
  ...
  try {
    ...
    const commitMessage = suggestedCommitMessage?.trim() || SANDBOX_AUTO_COMMIT_MESSAGE;
    execFileSync(git, ['commit', '-m', commitMessage, '--no-verify'], {
      cwd: projectPath,
      encoding: 'utf-8',
      stdio: ['pipe', 'pipe', 'pipe']
    ...
    return {
      committed: false,
      error: error instanceof Error ? error.message : 'Failed to auto-commit sandbox changes'
    };
  }
}
...
const IDE_DETECTION: Partial<Record<SupportedIDE, { name: string; paths: Record<string, string[]>; commands: Record<string, string> }>> = {
  ...
  antigravity: {
    name: 'Antigravity',
    paths: {
      darwin: ['/Applications/Antigravity.app', '~/.antigravity/antigravity/bin/antigravity'],
      win32: [],
      linux: ['~/.antigravity/antigravity/bin/antigravity']
    },
...
export async function mergeTaskWorktree(
  taskId: string,
  options: { noCommit?: boolean } | undefined,
  deps: WorktreeMergeDeps
): Promise<IPCResult<WorktreeMergeResult>> {
  const { pythonEnvManager, getMainWindow } = deps;
  ...
  try {
    ...
    if (!pythonEnvManager.isEnvReady()) {
      ...
      if (autoBuildSource) {
        ...
        return { success: false, error: 'Python environment not ready and Auto-iFlow source not found' };
      }
    }
    ...
    const isSandboxContext = Boolean(project.isSandbox || task.metadata?.sandbox);
    if (autoCommitSandboxGitignoreIfNeeded(project.path, isSandboxContext)) {
      debug('Auto-committed sandbox cleanup to keep main clean');
    }

    // Block merge if the main project has uncommitted changes (avoid false "merge conflicts")
    if (isGitWorkTree(project.path)) {
      try {
        const uncommittedFiles = getUncommittedFiles(project.path);
        if (uncommittedFiles.length > 0) {
          const fileCount = uncommittedFiles.length;
          debug('Merge blocked: main project has uncommitted changes:', fileCount);
          return {
            success: true,
            data: {
              success: false,
              message: `Merge blocked: main project has ${fileCount} uncommitted ${fileCount === 1 ? 'change' : 'changes'}. Commit or stash them and retry.`
            }
          };
        }
      } catch (e) {
        debug('Failed to check git status before merge:', e);
      }
    } else {
      debug('Project is a bare repository - skipping dirty main check');
    }

    // Use run.py --merge to handle the merge
    const sourcePath = getEffectiveSourcePath();
    if (!sourcePath) {
      return { success: false, error: 'Auto-iFlow source not found' };
    }

    const runScript = path.join(sourcePath, 'run.py');
    const specDir = path.join(project.path, project.autoBuildPath || DEFAULT_AUTO_BUILD_PATH, 'specs', task.specId);
    ...
    return new Promise((resolve) => {
      ...
      const handleProcessExit = async (code: number | null, signal: string | null = null) => {
        ...
        if (code === 0) {
            ...
            if (isGitWorkTree(project.path)) {
              try {
                ...
                if (!hasActualStagedChanges) {
                  ...
                  const specBranch = resolveTaskBranch(project.path, task.specId);
                  try {
          ...
          if (isStageOnly && !hasActualStagedChanges && mergeAlreadyCommitted) {
            ...
            try {
              if (worktreePath && existsSync(worktreePath)) {
                ...
                // Also delete the task branch (auto-iflow + legacy)
                deleteTaskBranches(project.path, task.specId, debug);
              }
            } catch (cleanupErr) {
            ...
            try {
              if (worktreePath && existsSync(worktreePath)) {
                ...
                // Also consider legacy branch cleanup since we merged successfully
                deleteTaskBranches(project.path, task.specId, debug);
              }
            } catch (cleanupErr) {
              debug('Worktree cleanup failed (non-fatal):', cleanupErr);
          ...
          // Read suggested commit message (used for staged flow and sandbox auto-commit)
          ...
          if (isSandboxContext) {
            const autoCommitResult = autoCommitSandboxResults(
              project.path,
              task.specId,
            ...
            if (autoCommitResult.committed) {
              const autoCommitNote = autoCommitResult.commitMessage
                ? `Sandbox changes committed: ${autoCommitResult.commitMessage}`
                : 'Sandbox changes committed';

              if (isStageOnly) {
                newStatus = 'done';
                planStatus = 'completed';
                message = autoCommitNote;
              } else {
                message = `${message}. ${autoCommitNote}`;
              }

              staged = false;

              // Clean up worktree after sandbox auto-commit to keep repo tidy
              if (isStageOnly) {
                try {
                  if (worktreePath && existsSync(worktreePath)) {
                    execFileSync(getToolPath('git'), ['worktree', 'remove', '--force', worktreePath], {
                    ...
                    debug('Worktree cleaned up after sandbox auto-commit:', worktreePath);

                    // Also delete the task branch since we merged successfully
                    deleteTaskBranches(project.path, task.specId, debug);
                  }
                } catch (cleanupErr) {
                  debug('Worktree cleanup failed after sandbox auto-commit (non-fatal):', cleanupErr);
                }
              }
            } else if (autoCommitResult.error) {
              debug('Sandbox auto-commit failed:', autoCommitResult.error);
            }
          }
          ...
          if (worktreePath) {
            const worktreeSpecDir = path.join(worktreePath, project.autoBuildPath || DEFAULT_AUTO_BUILD_PATH, 'specs', task.specId);
          ...
          const uiSuggestedCommitMessage = staged ? suggestedCommitMessage : undefined;
          ...
          resolve({
            ...
            data: {
              ...
              suggestedCommitMessage: uiSuggestedCommitMessage
            }
          });
        } else {
...
export function registerWorktreeHandlers(
   ...
   * Per-spec architecture: Each spec has its own worktree under the data directory
   */
  ipcMain.handle(
    ...
    async (_, taskId: string): Promise<IPCResult<WorktreeStatus>> => {
      try {
        ...
        // Find worktree under the data directory
        const worktreePath = findTaskWorktree(project.path, task.specId);
...
ipcMain.handle(
  ...
  ipcMain.handle(
    ...
    async (_, taskId: string, options?: { noCommit?: boolean }): Promise<IPCResult<WorktreeMergeResult>> =>
      mergeTaskWorktree(taskId, options, { pythonEnvManager, getMainWindow })
  );
      ...
      try {
        ...
        if (!pythonEnvManager.isEnvReady()) {
          ...
          if (autoBuildSource) {
            ...
            console.error('[IPC] Auto-iFlow source not found');
            return { success: false, error: 'Python environment not ready and Auto-iFlow source not found' };
          }
        }
        ...
        const isSandboxContext = Boolean(project.isSandbox || task.metadata?.sandbox);
        if (autoCommitSandboxGitignoreIfNeeded(project.path, isSandboxContext)) {
          console.warn('[IPC] Auto-committed sandbox .gitignore before preview');
        }

        if (isGitWorkTree(project.path)) {
          try {
            uncommittedFiles = getUncommittedFiles(project.path);
  ...
  ipcMain.handle(
    IPC_CHANNELS.TASK_WORKTREE_APPLY_FIX,
    async (_, taskId: string, action: WorktreeFixAction): Promise<IPCResult<WorktreeFixResult>> => {
      try {
        const { task, project } = findTaskAndProject(taskId);
        if (!task || !project) {
          return { success: false, error: 'Task not found' };
        ...
        const isSandboxContext = Boolean(project.isSandbox || task.metadata?.sandbox);
        if (!isSandboxContext) {
          return { success: false, error: 'Fix actions are only available for sandbox projects' };
        }

        if (!isGitWorkTree(project.path)) {
          return { success: false, error: 'Project is not a git working tree' };
        }

        const git = getToolPath('git');

        if (action === 'commit_gitignore') {
          const files = getUncommittedFiles(project.path);
          if (files.length !== 1 || files[0] !== '.gitignore') {
            return { success: false, error: 'Only .gitignore can be auto-committed' };
          }
          ...
          execFileSync(git, ['commit', '-m', SANDBOX_GITIGNORE_COMMIT_MESSAGE, '--no-verify'], {
            cwd: project.path,
            encoding: 'utf-8',
            stdio: ['pipe', 'pipe', 'pipe']
          });
          return { success: true, data: { success: true, message: 'Committed .gitignore' } };
        }

        if (action === 'stash_changes') {
          const files = getUncommittedFiles(project.path);
          if (files.length === 0) {
            return { success: false, error: 'No uncommitted changes to stash' };
          }
          execFileSync(git, ['stash', 'push', '-u', '-m', 'auto-iflow: sandbox cleanup'], {
            cwd: project.path,
            encoding: 'utf-8',
            stdio: ['pipe', 'pipe', 'pipe']
          });
          return { success: true, data: { success: true, message: 'Changes stashed' } };
        }
