...
const COMPLEXITY_MAP: Record<string, TaskMetadata['complexity']> = {
  simple: 'small',
  standard: 'medium',
  complex: 'complex'
};
...
export class ProjectStore {
  ...
  private reconcileAutoBuildPaths(): string[] {
    ...
    for (const project of this.data.projects) {
      ...
      const detectedPath = getAutoBuildPath(project.path);
      if (detectedPath && detectedPath !== project.autoBuildPath) {
   ...
   * Add a new project
   */
  addProject(projectPath: string, name?: string, options?: { isSandbox?: boolean }): Project {
    // Check if project already exists
    const existing = this.data.projects.find((p) => p.path === projectPath);
    if (existing) {
      if (options?.isSandbox && !existing.isSandbox) {
        existing.isSandbox = true;
        existing.updatedAt = new Date();
        this.save();
      }
      // Validate that data folder still exists for existing project
      ...
      if (existing.autoBuildPath && !isInitialized(existing.path)) {
        console.warn(`[ProjectStore] Data directory was deleted for project "${existing.name}" - resetting autoBuildPath`);
    ...
    // Determine data path (supports default and legacy)
  ...
  /**
   * Validate all projects to ensure their data folders still exist.
   ...
   *
   * @returns Array of project IDs that were reset due to missing data folder
   */
  validateProjects(): string[] {
    const resetProjectIds: string[] = [];
    let hasChanges = false;

    const updatedIds = this.reconcileAutoBuildPaths();
    if (updatedIds.length > 0) {
      console.warn(`[ProjectStore] Updated autoBuildPath for ${updatedIds.length} project(s)`);
    }
    ...
    for (const project of this.data.projects) {
      ...
      if (!isInitialized(project.path)) {
        console.warn(`[ProjectStore] Data folder missing for project "${project.name}" at ${project.path}`);
    ...
    if (hasChanges) {
      this.save();
      console.warn(`[ProjectStore] Reset ${resetProjectIds.length} project(s) due to missing data folder`);
    }
  ...
  private loadTasksFromSpecsDir(
    ...
    for (const dir of specDirs) {
      ...
      try {
        ...
        const assessmentPath = path.join(specPath, 'complexity_assessment.json');
        if (existsSync(assessmentPath)) {
          try {
            const content = readFileSync(assessmentPath, 'utf-8');
            const assessment = JSON.parse(content);
            const assessmentComplexity = assessment?.complexity;
            const assessmentRisk = assessment?.analysis?.risk?.level;
            const assessmentConfidence =
              typeof assessment?.confidence === 'number' ? assessment.confidence : undefined;

            if (!metadata) {
              metadata = {};
            }

            if (!metadata.complexity && typeof assessmentComplexity === 'string') {
              const mapped = COMPLEXITY_MAP[assessmentComplexity];
              if (mapped) {
                metadata.complexity = mapped;
              }
            }

            metadata.complexityAssessment = {
              complexity: typeof assessmentComplexity === 'string' ? assessmentComplexity : undefined,
              riskLevel: typeof assessmentRisk === 'string' ? assessmentRisk : undefined,
  ...
  private determineTaskStatusAndReason(
    ...
    const planReviewReason = plan
      ...
      : undefined;
    const normalizedPlanReviewReason = planReviewReason && ['completed', 'errors', 'qa_rejected', 'plan_review'].includes(planReviewReason)
      ? planReviewReason
      : undefined;
    ...
    if (allSubtasks.length > 0) {
      ...
      if (completed === allSubtasks.length) {
        ...
        // Some subtasks failed - mark task as failed
    ...
    if (plan?.status) {
      ...
      if (storedStatus) {
        ...
        const isStoredStatusValid =
          ...
          (storedStatus === 'failed') || // Failed should remain until manually updated
          (storedStatus === 'paused') || // Paused should remain until user resumes
        ...
        if (isStoredStatusValid) {
          ...
          if (storedStatus === 'human_review') {
            if (!reviewReason && normalizedPlanReviewReason) {
              reviewReason = normalizedPlanReviewReason;
            }
            if (!reviewReason) {
