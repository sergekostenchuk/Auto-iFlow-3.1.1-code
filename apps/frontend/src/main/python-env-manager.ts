...
/**
 * Manages the Python environment for the auto-iflow backend.
 *
 * For packaged apps:
...
export class PythonEnvManager extends EventEmitter {
  ...
  private hasBundledPackages(): boolean {
    ...
    const criticalPackages = ['iflow_sdk', 'dotenv'];
  ...
  private async checkDepsInstalled(): Promise<boolean> {
    ...
    try {
      ...
      // See: https://github.com/AndyMik90/Auto-iFlow/issues/359
      //
      // Dependencies checked:
      // - iflow_sdk: Core agent SDK (required)
...
`;
   ...
   *
   * @see https://github.com/AndyMik90/Auto-iFlow/issues/176
   */
