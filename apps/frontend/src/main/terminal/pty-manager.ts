...
export function setupPtyHandlers(
  ...
  terminal.ptyDisposables = { data: dataDisposable, exit: exitDisposable };
}
...
export function killPty(terminal: TerminalProcess): void {
  ...
  terminal.ptyDisposables?.data?.dispose();
  terminal.ptyDisposables?.exit?.dispose();
  terminal.ptyDisposables = undefined;
  terminal.pty.kill();
}
