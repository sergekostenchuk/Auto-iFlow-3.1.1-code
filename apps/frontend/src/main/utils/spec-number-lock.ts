...
export class SpecNumberLock {
  ...
  constructor(projectDir: string, autoBuildPath?: string) {
    this.projectDir = projectDir;
    this.autoBuildPath = autoBuildPath || DEFAULT_AUTO_BUILD_PATH;
    this.lockDir = path.join(projectDir, this.autoBuildPath, '.locks');
  ...
  getNextSpecNumber(autoBuildPath?: string): number {
    ...
    const specsBase = autoBuildPath || this.autoBuildPath || DEFAULT_AUTO_BUILD_PATH;
    ...
    const worktreesDir = path.join(this.projectDir, specsBase, 'worktrees', 'tasks');
...
export async function withSpecNumberLock<T>(
  ...
  const lock = new SpecNumberLock(projectDir, autoBuildPath);
  try {
    await lock.acquire();
