...
import { ImportTaskPlanView } from './components/ImportTaskPlanView';
...
import { ConsiliumView } from './components/consilium';
...
import type { Task, Project, ColorTheme, TabState } from '../shared/types';
import { ProjectTabBar } from './components/ProjectTabBar';
import { AddProjectModal } from './components/AddProjectModal';
import { StartupRestoreDialog } from './components/StartupRestoreDialog';
...
export function App() {
  ...
  const persistedTabState = useProjectStore((state) => state.persistedTabState);
  const openProjectTab = useProjectStore((state) => state.openProjectTab);
  const setTabState = useProjectStore((state) => state.setTabState);
  const setSelectedProjectId = useProjectStore((state) => state.setSelectedProjectId);
  const setActiveProject = useProjectStore((state) => state.setActiveProject);
  const setTabStatePersistenceEnabled = useProjectStore((state) => state.setTabStatePersistenceEnabled);
  const reorderTabs = useProjectStore((state) => state.reorderTabs);
  const tasks = useTaskStore((state) => state.tasks);
  const settings = useSettingsStore((state) => state.settings);
  const settingsLoading = useSettingsStore((state) => state.isLoading);
  const startupRestoreMode = settings.startupRestoreMode ?? 'ask';
  ...
  const [startupRestoreState, setStartupRestoreState] = useState<'pending' | 'restored' | 'fresh' | 'skipped'>('pending');
  const [showStartupRestoreDialog, setShowStartupRestoreDialog] = useState(false);
  ...
  // GitHub setup state (shown after Auto-iFlow init)
  ...
  const handleRestoreSelection = useCallback((tabState: TabState) => {
    setTabStatePersistenceEnabled(true);
    setTabState(tabState);
    if (tabState.activeProjectId) {
      useProjectStore.getState().selectProject(tabState.activeProjectId);
    } else {
      setSelectedProjectId(null);
    }
    setShowStartupRestoreDialog(false);
    setStartupRestoreState('restored');
  }, [setSelectedProjectId, setTabState, setTabStatePersistenceEnabled]);

  const handleStartFresh = useCallback(() => {
    setTabStatePersistenceEnabled(false);
    setTabState({ openProjectIds: [], activeProjectId: null, tabOrder: [] });
    setSelectedProjectId(null);
    setShowStartupRestoreDialog(false);
    setStartupRestoreState('fresh');
  }, [setSelectedProjectId, setTabState, setTabStatePersistenceEnabled]);
  ...
  useEffect(() => {
    const initializeApp = async () => {
      await loadSettings();
      await loadProfiles();
      const mode = useSettingsStore.getState().settings.startupRestoreMode ?? 'ask';
      await loadProjects({ applyTabState: mode === 'always' });
    };
    initializeApp();
  ...
  // Startup restore decision (prompt/auto/skip)
  useEffect(() => {
    if (settingsLoading || projectLoading) return;
    if (startupRestoreState !== 'pending') return;

    const hasSavedTabs = !!persistedTabState?.openProjectIds?.length;
    if (!hasSavedTabs) {
      setStartupRestoreState('skipped');
      return;
    }

    if (startupRestoreMode === 'always' && persistedTabState) {
      handleRestoreSelection(persistedTabState);
      return;
    }

    if (startupRestoreMode === 'never') {
      handleStartFresh();
      return;
    }

    setShowStartupRestoreDialog(true);
  }, [
    settingsLoading,
    projectLoading,
    startupRestoreState,
  ...
  useEffect(() => {
    ...
    if (startupRestoreState === 'pending' && startupRestoreMode === 'ask') {
      console.log('[App] Startup restore pending; skipping auto tab open');
      return;
    }

    if (startupRestoreState === 'fresh') {
      console.log('[App] Startup restore set to fresh; skipping auto tab open');
      return;
    }
  ...
  // Check if selected project needs initialization (e.g., .auto-iflow folder was deleted)
  useEffect(() => {
  ...
  const handleInitialize = async () => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        const errorMessage = result?.error || 'Failed to initialize Auto-iFlow. Please try again.';
  ...
  const handleImportTaskPlanClick = () => {
    setSettingsInitialProjectSection('plan-import');
    setIsSettingsDialogOpen(true);
  };
      ...
      <TooltipProvider>
        ...
        <div className="flex h-screen bg-background">
          ...
          <Sidebar
            ...
            onImportTaskPlanClick={handleImportTaskPlanClick}
            ...
            <main className="flex-1 overflow-hidden">
              {selectedProject ? (
                <>
                  ...
                  {activeView === 'import-task-plan' && (activeProjectId || selectedProjectId) && (
                    <ImportTaskPlanView projectId={activeProjectId || selectedProjectId!} />
                  )}
                  ...
                  {activeView === 'consilium' && (activeProjectId || selectedProjectId) && (
                    <ConsiliumView projectId={activeProjectId || selectedProjectId!} />
                  )}
                  {activeView === 'tetris' && <TetrisGame />}
                </>
              ) : (
                <WelcomeScreen
                  projects={projects}
          ...
          {/* Dialogs */}
          {(activeProjectId || selectedProjectId || settings.sandboxTasksEnabled) && (
            <TaskCreationWizard
          ...
          <StartupRestoreDialog
            open={showStartupRestoreDialog}
            projects={projects}
          ...
          {/* GitHub Setup Modal - shows after Auto-iFlow init to configure GitHub */}
          {gitHubSetupProject && (
            <GitHubSetupModal
