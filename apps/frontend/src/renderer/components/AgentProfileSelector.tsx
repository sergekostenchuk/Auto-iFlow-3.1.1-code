...
export function AgentProfileSelector({
  ...
  const handleProfileSelect = (selectedId: string) => {
    if (selectedId === 'custom') {
      ...
      onProfileChange('custom', model as ModelType || 'glm-4.7', thinkingLevel as ThinkingLevel || 'medium');
    } else {
        ...
        >
          ...
          <SelectContent>
            {DEFAULT_AGENT_PROFILES.map((profile) => {
              ...
              const modelLabel = AVAILABLE_CODING_MODELS.find(m => m.value === profile.model)?.label;
              return (
            ...
            <div className="px-4 pb-4 -mt-1">
              <div className="grid grid-cols-2 gap-2 text-xs">
                {(Object.keys(PHASE_LABEL_KEYS) as Array<keyof PhaseModelConfig>).map((phase) => {
                  const modelLabel = AVAILABLE_CODING_MODELS.find(m => m.value === currentPhaseModels[phase])?.label || currentPhaseModels[phase];
                  return (
