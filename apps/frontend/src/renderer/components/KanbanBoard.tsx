...
import { Plus, Inbox, Loader2, Eye, CheckCircle2, AlertTriangle, Archive, RefreshCw, PauseCircle } from 'lucide-react';
...
const getEmptyStateContent = (status: TaskStatus, t: (key: string) => string): { icon: React.ReactNode; message: string; subtext?: string } => {
  switch (status) {
    ...
    case 'paused':
      return {
        icon: <PauseCircle className="h-6 w-6 text-muted-foreground/50" />,
        message: t('kanban.emptyPaused'),
        subtext: t('kanban.emptyPausedHint')
      };
    case 'ai_review':
      return {
    ...
    case 'failed':
      return {
        ...
        message: t('kanban.emptyFailed'),
        subtext: t('kanban.emptyFailedHint')
      };
    case 'done':
      return {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-h-0">
        <ScrollArea className="kanban-scroll h-full px-3 pb-3 pt-2" viewportClassName="flex flex-col items-stretch w-full min-w-0">
          <SortableContext
            items={taskIds}
