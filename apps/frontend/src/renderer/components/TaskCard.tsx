...
export const TaskCard = memo(function TaskCard({ task, onClick, onStatusChange }: TaskCardProps) {
  ...
  const showPausedStatus = task.status === 'paused' || (task.status === 'done' && isImportController && task.metadata?.planImportQueue);
  ...
  return (
    <Card
      className={cn(
        'card-surface task-card-enhanced cursor-pointer w-full min-w-0 max-w-none',
        ...
        {(task.metadata || isStuck || isIncomplete || hasActiveExecution || reviewReasonInfo) && (
          <div className="mt-2.5 flex flex-wrap gap-1.5">
            ...
            {!hasActiveExecution && (
              ...
              >
                {isStuck
                  ? t('labels.needsRecovery')
                  : isIncomplete
                    ? t('labels.needsResume')
                    : showPausedStatus
                      ? t('labels.paused')
