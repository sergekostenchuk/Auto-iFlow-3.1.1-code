  ...
  useTranslation: () => ({
    t: (key: string) => {
      ...
      const translations: Record<string, string> = {
        'welcome.title': 'Welcome to Auto-iFlow',
        'welcome.subtitle': 'AI-powered autonomous coding assistant',
        'welcome.getStarted': 'Get Started',
        'welcome.skip': 'Skip Setup',
        'wizard.helpText': 'Let us help you get started with Auto-iFlow',
        'welcome.features.aiPowered.title': 'AI-Powered',
        'welcome.features.aiPowered.description': 'Powered by iFlow',
        ...
        'authChoice.oauthTitle': 'Sign in via iFlow',
        'authChoice.oauthDesc': 'OAuth authentication',
        'authChoice.apiKeyTitle': 'Use API Key',
...
Object.defineProperty(window, 'electronAPI', {
  value: {
    ...
    checkIflowStatus: vi.fn().mockResolvedValue({
      success: true,
      data: { installed: '0.1.0', hasApiKey: true, hasWebLogin: false }
    }),
    startIflowWebLogin: vi.fn().mockResolvedValue({ success: true, data: { opened: true } })
  },
  writable: true
});
...
describe('OnboardingWizard Integration Tests', () => {
  ...
  describe('OAuth Path Navigation', () => {
    ...
    it.skip('should navigate: welcome → auth-choice → oauth', async () => {
      ...
      expect(screen.getByText(/Welcome to Auto-iFlow/)).toBeInTheDocument();
  ...
  describe('API Key Path Navigation', () => {
    ...
    it.skip('should skip oauth step when API key path chosen', async () => {
      ...
      // API key dialog should open
      await waitFor(() => {
        expect(screen.getByText(/Enter iFlow API Key/)).toBeInTheDocument();
        expect(screen.getByPlaceholderText(/iflow-/)).toBeInTheDocument();
      });
  ...
  describe('First-Run Detection', () => {
    ...
    it('should not show wizard for users with existing OAuth', () => {
      ...
      expect(screen.queryByText(/Welcome to Auto-iFlow/)).not.toBeInTheDocument();
    });
