...
export type AppSection = 'appearance' | 'display' | 'language' | 'devtools' | 'agent' | 'model-routing' | 'paths' | 'integrations' | 'api-profiles' | 'updates' | 'notifications' | 'debug';
...
const appNavItemsConfig: NavItemConfig<AppSection>[] = [
  ...
  { id: 'agent', icon: Bot },
  { id: 'model-routing', icon: Sparkles },
...
export function AppSettingsDialog({ open, onOpenChange, initialSection, initialProjectSection, onRerunWizard }: AppSettingsDialogProps) {
  ...
  const renderAppSection = () => {
    switch (appSection) {
      ...
      case 'model-routing':
        return <ModelRoutingSettings settings={settings} onSettingsChange={setSettings} />;
      case 'paths':
