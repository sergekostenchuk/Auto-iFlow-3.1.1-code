...
import type { AppSettings, StartupRestoreMode } from '../../../shared/types';
...
export function DisplaySettings({ settings, onSettingsChange }: DisplaySettingsProps) {
  ...
  const startupRestoreMode: StartupRestoreMode = settings.startupRestoreMode ?? 'ask';
  ...
  const handleStartupRestoreChange = (value: StartupRestoreMode) => {
    onSettingsChange({ ...settings, startupRestoreMode: value });
  };
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-3 pt-2">
          <Label className="text-sm font-medium text-foreground">
            {t('startupRestore.title')}
          </Label>
          <p className="text-sm text-muted-foreground">
            {t('startupRestore.description')}
          </p>
          <Select value={startupRestoreMode} onValueChange={handleStartupRestoreChange}>
            <SelectTrigger className="w-full max-w-md">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="always">{t('startupRestore.options.always')}</SelectItem>
              <SelectItem value="ask">{t('startupRestore.options.ask')}</SelectItem>
              <SelectItem value="never">{t('startupRestore.options.never')}</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
