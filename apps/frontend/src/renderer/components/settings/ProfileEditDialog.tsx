...
import type { APIProfile, ProfileFormData, TestConnectionResult, CustomModel } from '@shared/types/profile';
...
export function ProfileEditDialog({ open, onOpenChange, onSaved, profile }: ProfileEditDialogProps) {
  ...
  const handleSave = async () => {
    ...
    const normalizedCustomModels = customModels
      .map((model) => {
        ...
        const aliases = (model.aliases ?? []).map((alias) => alias.trim()).filter(Boolean);
        return {
          ...model,
          id,
          displayName,
    ...
    if (isEditMode && profile) {
      ...
      const updatedProfile: APIProfile = {
        ...
        customModels: normalizedCustomModels.length > 0 ? normalizedCustomModels : undefined
      };
  ...
  const updateCustomModel = (index: number, updates: Partial<CustomModel>) => {
    setCustomModels((current) =>
      current.map((model, idx) => (idx === index ? { ...model, ...updates } : model))
    );
  };

  const removeCustomModel = (index: number) => {
    setCustomModels((current) => current.filter((_, idx) => idx !== index));
  };
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-3 pt-2 border-t">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <Label className="text-base">{t('settings:apiProfiles.customModels.title')}</Label>
                <p className="text-xs text-muted-foreground">
                  {t('settings:apiProfiles.customModels.description')}
                </p>
              </div>
              <Button type="button" variant="outline" size="sm" onClick={addCustomModel}>
                <Plus className="mr-2 h-4 w-4" />
                {t('settings:apiProfiles.customModels.add')}
              </Button>
            </div>

            {customModels.length === 0 ? (
              <p className="text-xs text-muted-foreground">
                {t('settings:apiProfiles.customModels.empty')}
              </p>
            ) : (
              <div className="space-y-3">
                {customModels.map((model, index) => (
                  <div key={`${model.id}-${index}`} className="rounded-md border border-border/60 p-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium">
                        {t('settings:apiProfiles.customModels.itemTitle', { index: index + 1 })}
                      </span>
                      <Button
                        type="button"
                        variant="ghost"
                        ...
                        aria-label={t('settings:apiProfiles.customModels.remove')}
                      >
                    ...
                    <div className="mt-3 grid gap-3 md:grid-cols-2">
                      <div className="space-y-2">
                        <Label className="text-sm text-muted-foreground">
                          {t('settings:apiProfiles.customModels.fields.id')}
                        </Label>
                        <Input
                          value={model.id}
                          onChange={(e) => updateCustomModel(index, { id: e.target.value })}
                          placeholder={t('settings:apiProfiles.customModels.placeholders.id')}
                        />
                      </div>

                      <div className="space-y-2">
                        <Label className="text-sm text-muted-foreground">
                          {t('settings:apiProfiles.customModels.fields.displayName')}
                        </Label>
                        <Input
                          value={model.displayName}
                          onChange={(e) => updateCustomModel(index, { displayName: e.target.value })}
                          placeholder={t('settings:apiProfiles.customModels.placeholders.displayName')}
                        />
                      </div>

                      <div className="space-y-2">
                        <Label className="text-sm text-muted-foreground">
                          {t('settings:apiProfiles.customModels.fields.tier')}
                        </Label>
                        <Select
                          value={model.tier}
                          onValueChange={(value) =>
                            updateCustomModel(index, { tier: value as CustomModel['tier'] })
                          }
                        >
                          <SelectTrigger>
                            <SelectValue placeholder={t('settings:apiProfiles.customModels.placeholders.tier')} />
                          </SelectTrigger>
                          <SelectContent>
                            {['economy', 'balanced', 'premium', 'experimental'].map((tier) => (
                              <SelectItem key={tier} value={tier}>
                                {t(`settings:apiProfiles.customModels.tiers.${tier}`)}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      ...
                      <div className="space-y-2">
                        <Label className="text-sm text-muted-foreground">
                          {t('settings:apiProfiles.customModels.fields.supportsThinking')}
                        </Label>
                        <div className="flex items-center justify-between rounded-md border border-border/60 px-3 py-2">
                          <span className="text-xs text-muted-foreground">
                            {model.supportsThinking
                              ? t('settings:apiProfiles.customModels.supportsThinking.on')
                              : t('settings:apiProfiles.customModels.supportsThinking.off')}
                          </span>
                          <Switch
                            checked={model.supportsThinking}
                            onCheckedChange={(checked) =>
                              updateCustomModel(index, { supportsThinking: checked })
                            }
                          />
                        </div>
                      </div>
                    ...
                    <div className="mt-3 space-y-2">
                      <Label className="text-sm text-muted-foreground">
                        {t('settings:apiProfiles.customModels.fields.aliases')}
                      </Label>
                      <Input
                        ...
                        placeholder={t('settings:apiProfiles.customModels.placeholders.aliases')}
                      />
                    </div>
                  </div>
                ))}
