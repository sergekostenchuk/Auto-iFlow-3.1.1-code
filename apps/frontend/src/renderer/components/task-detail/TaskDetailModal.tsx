...
import { TaskModelRouting } from './TaskModelRouting';
import { TaskQualityChecks } from './TaskQualityChecks';
import { TaskWarnings } from './TaskWarnings';
import { TaskSubtasks } from './TaskSubtasks';
import { TaskLogs } from './TaskLogs';
import { TaskFiles } from './TaskFiles';
import { TaskReview } from './TaskReview';
import type { Task, PostCodeTestsReport } from '../../../shared/types';
...
function TaskDetailModalContent({ open, task, onOpenChange, onSwitchToTerminals, onOpenInbuiltTerminal }: { open: boolean; task: Task; onOpenChange: (open: boolean) => void; onSwitchToTerminals?: () => void; onOpenInbuiltTerminal?: (id: string, cwd: string) => void }) {
  ...
  const [postCodeTestsLoaded, setPostCodeTestsLoaded] = useState(false);
  const [allowMergeWithoutTests, setAllowMergeWithoutTests] = useState(false);
  const autoMergeEnabled = task.metadata?.requireReviewBeforeMerge === false &&
    (!task.reviewReason || task.reviewReason === 'completed');

  useEffect(() => {
    let cancelled = false;
    const loadPostCodeTests = async () => {
      if (!task.specsPath) {
        setPostCodeTests(null);
        setPostCodeTestsLoaded(false);
        return;
      }
      try {
        const normalized = task.specsPath.replace(/\\/g, '/');
        const filePath = normalized.endsWith('/') ? `${normalized}post_code_tests.json` : `${normalized}/post_code_tests.json`;
        const result = await window.electronAPI.readFile(filePath);
        if (cancelled) return;
        if (result.success && result.data !== undefined) {
          setPostCodeTests(JSON.parse(result.data));
        } else {
          setPostCodeTests(null);
        }
        setPostCodeTestsLoaded(true);
      } catch (error) {
        if (!cancelled) {
          setPostCodeTests(null);
          setPostCodeTestsLoaded(true);
  ...
  const handleResumeImportQueue = async () => {
    try {
      const response = await window.electronAPI.resumeTaskPlanImport(task.id);
      if (!response.success) {
        throw new Error(response.error || 'Failed to resume task plan import');
      }
      toast({
        title: 'Import queue resumed',
        description: 'Task plan execution has resumed from the last paused step.',
        duration: 3000
      });
    } catch (error) {
      toast({
        title: 'Failed to resume',
        description: error instanceof Error ? error.message : 'Unable to resume task plan import.',
        variant: 'destructive',
        duration: 4000
      });
    }
  ...
  const runMerge = async (noCommit: boolean, allowStageSuccess: boolean) => {
    state.setIsMerging(true);
    state.setWorkspaceError(null);
    try {
      const result = await window.electronAPI.mergeWorktree(task.id, { noCommit });
      if (result.success && result.data?.success) {
        if (allowStageSuccess && noCommit && result.data.staged) {
          state.setWorkspaceError(null);
  ...
  const handleMerge = async () => {
    await runMerge(state.stageOnly, true);
  };

  const handleAutoMerge = async () => {
    await runMerge(false, false);
  };
  ...
  const isImportController = task.metadata?.planImportId === task.id;
  const canResumeImportQueue = Boolean(task.metadata?.planImportQueue && isImportController);
  ...
  const renderPrimaryAction = () => {
    ...
    if (canResumeImportQueue) {
      return (
        <Button variant="default" onClick={handleResumeImportQueue}>
          <Play className="mr-2 h-4 w-4" />
          Resume Queue
        </Button>
      );
  ...
  return (
    <TooltipProvider delayDuration={300}>
                  ...
                  <DialogPrimitive.Description asChild>
                    <div className="mt-2.5 flex items-center gap-2 flex-wrap">
                      ...
                      {state.isStuck ? (
                        ...
                        <>
                          <Badge
                            variant={task.status === 'done' ? 'success' : task.status === 'human_review' ? 'purple' : task.status === 'failed' ? 'destructive' : task.status === 'in_progress' ? 'info' : 'secondary'}
                ...
                <TabsContent value="overview" className="flex-1 min-h-0 overflow-hidden mt-0">
                  <ScrollArea className="h-full">
                    <div className="p-5 space-y-5 overflow-x-hidden max-w-full">
                      ...
                      <Separator />
                      <TaskModelRouting task={task} />
                      <Separator />
                      <TaskQualityChecks
                        task={task}
                        allowMergeWithoutTests={allowMergeWithoutTests}
                        onAllowMergeWithoutTestsChange={setAllowMergeWithoutTests}
                      />
      ...
      <TaskReview
        ...
        postCodeTests={postCodeTests}
        postCodeTestsLoaded={postCodeTestsLoaded}
        allowMergeWithoutTests={allowMergeWithoutTests}
        onAllowMergeWithoutTestsChange={setAllowMergeWithoutTests}
