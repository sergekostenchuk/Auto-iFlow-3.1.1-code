...
export function useTaskDetail({ task }: UseTaskDetailOptions) {
  ...
  const taskProject = useProjectStore((state) =>
    state.projects.find((project) => project.id === task.projectId)
  );
  const projectForLogs = taskProject ?? selectedProject;
  ...
  useEffect(() => {
    if (!projectForLogs) return;

    const loadLogs = async () => {
      setIsLoadingLogs(true);
      try {
        const result = await window.electronAPI.getTaskLogs(projectForLogs.id, task.specId);
    ...
    window.electronAPI.watchTaskLogs(projectForLogs.id, task.specId);
