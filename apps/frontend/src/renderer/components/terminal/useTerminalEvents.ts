...
export function useTerminalEvents({
  ...
  // Buffer for chunked output to reduce UI jitter
  ...
  const flushBuffer = useRef(() => {
    if (bufferRef.current.length > 0) {
      terminalBufferManager.append(terminalId, bufferRef.current);
      onOutputRef.current?.(bufferRef.current);
      bufferRef.current = '';
    }
  }).current;
  ...
  useEffect(() => {
    ...
    return cleanup;
  }, [terminalId, flushBuffer]);

  // Flush buffer on unmount to ensure all output is displayed
  useEffect(() => {
    return () => {
      flushBuffer();
    };
