...
import type { Task, TaskStatus, ImplementationPlan, Subtask, TaskMetadata, ExecutionProgress, ExecutionPhase, ReviewReason, TaskDraft, SandboxTaskCreateResult } from '../../shared/types';
      ...
      return {
        tasks: updateTaskAtIndex(state.tasks, index, (t) => {
          ...
          if (status === 'backlog') {
            ...
            executionProgress = {
              phase: 'failed' as ExecutionPhase,
              phaseProgress: executionProgress?.phaseProgress || 0,
              overallProgress: executionProgress?.overallProgress || 0,
              message: executionProgress?.message || 'Task failed.'
            };
...
/**
 * Create a new sandbox task (isolated project)
 */
...
): Promise<SandboxTaskCreateResult | null> {
  const store = useTaskStore.getState();

  try {
    const result = await window.electronAPI.createSandboxTask(title, description, metadata, sourceProjectId);
    if (result.success && result.data) {
      return result.data;
    }
    store.setError(result.error || 'Failed to create sandbox task');
    return null;
  } catch (error) {
    store.setError(error instanceof Error ? error.message : 'Unknown error');
    return null;
...
export async function deleteTask(
  ...
  try {
    ...
    if (result.success) {
      if (result.data?.removedProjectId) {
        const projectStore = useProjectStore.getState();
        projectStore.removeProject(result.data.removedProjectId);
        if (projectStore.openProjectIds.includes(result.data.removedProjectId)) {
          projectStore.closeProjectTab(result.data.removedProjectId);
        }
      }
