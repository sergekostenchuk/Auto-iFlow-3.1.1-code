...
import type {
  ...
  TaskMetadata,
  IntakeResult,
  IntakeMetricsSummary,
  ClarifyAnswers,
  TaskLogs,
  TaskLogStreamChunk,
  TaskPlanImportResult,
  TaskPlanImportResumeResult,
...
import type { AppSettings, SourceEnvConfig, SourceEnvCheckResult, ModelRegistryData, ModelRoutingSettings } from './settings';
...
export interface SandboxTaskCreateResult {
  task: Task;
  project: Project;
}
...
export interface ElectronAPI {
  ...
  createSandboxTask: (title: string, description: string, metadata?: TaskMetadata, sourceProjectId?: string) => Promise<IPCResult<SandboxTaskCreateResult>>;
  importTaskPlan: (
    projectId: string,
    planPath: string,
    options?: TaskPlanImportOptions
  ) => Promise<IPCResult<TaskPlanImportResult>>;
  resumeTaskPlanImport: (taskId: string) => Promise<IPCResult<TaskPlanImportResumeResult>>;
  runIntakeAnalysis: (
    projectId: string,
    description: string,
    model: string,
    ...
    requestId?: string,
    iteration?: number
  ) => Promise<IPCResult<IntakeResult>>;
  runIntakeReanalyze: (
    projectId: string,
    description: string,
    model: string,
  ...
  getIntakeMetrics: () => Promise<IPCResult<IntakeMetricsSummary>>;
  ...
  applyWorktreeFix: (taskId: string, action: WorktreeFixAction) => Promise<IPCResult<WorktreeFixResult>>;
  ...
  getModelRegistry: () => Promise<IPCResult<ModelRegistryData>>;
  ...
  // Auto-iFlow source environment operations
  ...
  checkIflowStatus: () => Promise<IPCResult<import('./cli').IFlowCliStatus>>;
  ...
  consiliumStart: (task: string, projectId?: string, model?: string) => Promise<IPCResult>;
  consiliumStop: () => Promise<IPCResult>;
  consiliumApprove: () => Promise<IPCResult>;
  consiliumReject: (feedback: string) => Promise<IPCResult>;
  consiliumGetStatus: () => Promise<IPCResult<{
