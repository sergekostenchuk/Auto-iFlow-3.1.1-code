...
import type { ModelRoutingSettings } from './settings';
...
export interface GraphitiProviderConfig {
  ...
  database?: string;  // Database name (default: auto_iflow_memory)
  dbPath?: string;    // Database storage path (default: ~/.auto-iflow/graphs)
}
...
export interface ProjectEnvConfig {
  ...
  // Model routing (project-level overrides)
  modelRouting?: ModelRoutingSettings;
 ...
 * Stored in .auto-iflow/.env as AGENT_MCP_<agent>_ADD and AGENT_MCP_<agent>_REMOVE
 */
...
// Auto-iFlow Initialization Types
export interface AutoBuildVersionInfo {
  isInitialized: boolean;
  updateAvailable: boolean; // Always false - .auto-iflow only contains data, no code to update
}
