...
// Thinking level for models (budget token allocation)
export type ThinkingLevel = 'none' | 'low' | 'medium' | 'high' | 'ultrathink';

// Startup behavior for restoring tabs/projects
export type StartupRestoreMode = 'always' | 'ask' | 'never';

// iFlow model IDs for coding pipeline
export type IFlowModelId =
  | 'glm-4.7'
  | 'iflow-rome-30ba3b'
  | 'deepseek-v3.2'
  | 'qwen3-coder-plus'
...
// Model type shorthand / IDs
export type ModelTypeShort = 'haiku' | 'sonnet' | 'opus' | IFlowModelId;
...
export interface ModelChoice {
  model: ModelTypeShort | null; // null = inherit
  thinkingLevel: ThinkingLevel | null; // null = inherit
}

export interface ModelRoutingPhases {
  spec: ModelChoice;
  planning: ModelChoice;
  coding: ModelChoice;
  validation: ModelChoice;
}

export interface ModelRoutingFeatures {
  consilium: ModelChoice;
  insights: ModelChoice;
  ideation: ModelChoice;
  github: ModelChoice;
  merge: ModelChoice;
  commit: ModelChoice;
  intake: ModelChoice;
...
export interface ModelRoutingAdvancedRoles {
  consilium?: {
    innovator?: ModelChoice;
    realist?: ModelChoice;
    facilitator?: ModelChoice;
  };
  github?: {
    review?: ModelChoice;
    followUp?: ModelChoice;
    batch?: ModelChoice;
  };
  insights?: {
    extractor?: ModelChoice;
    summarizer?: ModelChoice;
  };
}

export interface ModelRoutingSettings {
  phases: ModelRoutingPhases;
  features: ModelRoutingFeatures;
  advancedRoles?: ModelRoutingAdvancedRoles;
}
...
export type ModelTier = 'economy' | 'balanced' | 'premium' | 'experimental';
...
export interface AppSettings {
  ...
  globalIflowApiKey?: string;
  iflowOAuthResetAt?: number;
  ...
  // Model routing settings (global defaults)
  modelRouting?: ModelRoutingSettings;
  // Persist advanced role visibility for model routing
  modelRoutingAdvanced?: boolean;
  ...
  uiScale?: number;
  // Startup restore behavior for project tabs
  startupRestoreMode?: StartupRestoreMode;
  // Sandbox task isolation (1 task = 1 repo)
  sandboxTasksEnabled?: boolean;
  // Strict Intake V2 rollout switch
  intakeV2Enabled?: boolean;
...
// Auto-iFlow Source Environment Configuration (for Auto-iFlow repo .env)
