...
export type TaskStatus = 'backlog' | 'in_progress' | 'paused' | 'ai_review' | 'human_review' | 'failed' | 'done';
...
// Post-code test report (stored in post_code_tests.json)
export interface PostCodeTestResultItem {
  command: string;
  status: string;
...
export interface PostCodeTestsReport {
  status?: string;
  summary?: { total?: number; passed?: number; failed?: number };
  test_plan?: string[];
  results?: PostCodeTestResultItem[];
  started_at?: string;
  completed_at?: string;
  commit?: string | null;
  reason?: string;
...
export interface PostCodeTestsRunResult {
  report: PostCodeTestsReport;
}

export interface PostCodeTestsFixResult {
  applied: boolean;
  message: string;
  report?: PostCodeTestsReport;
}

export interface PostCodeTestsFixOptions {
  upgradePip?: boolean;
}
...
// Intake (Strict Task Creation) types
export type IntakeClarityLevel = 'high' | 'medium' | 'low';
...
export interface IntakeResult {
  clarityLevel: IntakeClarityLevel;
...
export type IntakeOutcome = 'completed' | 'needs_clarify' | 'blocked' | 'error';

export interface IntakeMetricEvent {
  request_id: string;
  round: number;
  clarity: IntakeClarityLevel;
  blockers_count: number;
  duration_ms: number;
  force_high_triggered: boolean;
  outcome: IntakeOutcome;
  timestamp: string;
}

export interface IntakeMetricsSummary {
  window_days: number;
  total_events: number;
  high_events: number;
  blocked_events: number;
  force_high_events: number;
  high_rate_pct: number;
  blocked_rate_pct: number;
  force_high_rate_pct: number;
  avg_round: number;
}
...
export interface PlanImportPipelineAgents {
...
export interface PlanImportPipelineMetadata {
  enabled?: boolean;
  mode?: 'agent' | 'deterministic';
  agents?: PlanImportPipelineAgents;
}

export interface PlanImportQueueGroup {
  parallel: boolean;
  taskIds: string[];
}

export interface PlanImportQueueState {
  status?: 'running' | 'paused' | 'completed';
  cursor?: number;
  groups?: PlanImportQueueGroup[];
...
export interface TaskMetadata {
  ...
  requireReviewBeforeMerge?: boolean;  // Require human review before merging changes to main
  ralphLoop?: boolean;  // Optional Ralph-loop (iterate until passes=true)

  // Sandbox isolation
  sandbox?: boolean;

  // Agent configuration (from agent profile or manual selection)
  model?: ModelType;  // Model to use (legacy shorthands or iFlow IDs) - used when not auto profile
  ...
  // New model routing (UI editable) and snapshot (read-only for runners)
  modelRouting?: ModelRoutingSettings;
  resolvedModel?: ResolvedModelSnapshot;
  ...
  // Plan Importer (agent pipeline metadata)
  planImportPipeline?: PlanImportPipelineMetadata;
  planImportId?: string;
  planImportQueue?: PlanImportQueueState;

  // Strict Intake metadata
...
// Implementation Plan (Auto-iFlow format)
...
export type WorktreeFixAction = 'commit_gitignore' | 'stash_changes';

export interface WorktreeFixResult {
  success: boolean;
  message: string;
}
...
export interface TaskPlanImportResult {
...
export interface TaskPlanImportResumeResult {
  resumed: boolean;
  cursor?: number;
  status?: PlanImportQueueState['status'];
}
...
export interface TaskPlanImportOptions {
  autoStart?: boolean;
