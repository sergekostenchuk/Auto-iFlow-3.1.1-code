# Auto-iFlow CLI Usage

This document covers terminal-only usage of Auto-iFlow. **For most users, we recommend using the [Desktop UI](#) instead** - it provides a better experience with visual task management, progress tracking, and automatic Python environment setup.

## When to Use CLI

- You prefer terminal workflows
- You're running on a headless server
- You're integrating Auto-iFlow into scripts or CI/CD
...
Auto-iFlow uses Git worktrees for isolated builds:

```bash
# Test the feature in the isolated workspace
cd .auto-iflow/worktrees/tasks/<spec-name>/  # legacy: .worktrees/<spec-name>/
