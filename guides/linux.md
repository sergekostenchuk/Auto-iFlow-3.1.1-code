...
```bash
flatpak install --user apps/frontend/dist/Auto-iFlow-*.flatpak
```
...
```bash
# Make executable
chmod +x Auto-iFlow-*-linux-x86_64.AppImage

# Run
./Auto-iFlow-*-linux-x86_64.AppImage
```
...
```bash
sudo dpkg -i Auto-iFlow-*-linux-amd64.deb
```
...
ldd ./Auto-iFlow-*-linux-x86_64.AppImage

# Try running with debug output
./Auto-iFlow-*-linux-x86_64.AppImage --verbose
```
