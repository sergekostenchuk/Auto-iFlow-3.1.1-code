...
# =============================================================================
# PRE-MOCK EXTERNAL SDK MODULES - Must happen BEFORE adding auto-iflow to path
# =============================================================================
...
def pytest_runtest_setup(item):
    ...
    preserved_mocks = module_mocks.get(module_basename, set())
    ...
    # Ensure preserved mocks are present if the test module defines them
    mock_attr_map = {
        'progress': 'mock_progress',
        'ui': 'mock_ui',
        'task_logger': 'mock_task_logger',
        'linear_updater': 'mock_linear',
        'client': 'mock_client',
        'claude_agent_sdk': 'mock_sdk',
        'claude_code_sdk': 'mock_sdk',
        'claude_code_sdk.types': 'mock_sdk',
        'claude_agent_sdk.types': 'mock_sdk',
    }
    for module_name in preserved_mocks:
        attr_name = mock_attr_map.get(module_name)
        if attr_name and hasattr(item.module, attr_name):
            sys.modules[module_name] = getattr(item.module, attr_name)

    # If we cleaned up mocks, we need to reload modules that might have cached
    # references to the mocked versions
    if cleaned_up and (module_basename.startswith("test_qa_") or module_basename.startswith("test_review")):
        # Reload progress first
        if 'progress' in sys.modules:
            progress_module = sys.modules['progress']
            # Only reload real modules; MagicMock does not support importlib.reload
            if isinstance(progress_module, types.ModuleType):
