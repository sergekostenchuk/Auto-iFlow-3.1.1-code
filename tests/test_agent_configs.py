...
class TestAgentConfigs:
    ...
    def test_all_agent_types_have_required_fields(self):
        """Every agent config should have tools, mcp_servers, auto_iflow_tools, thinking_default."""
        from agents.tools_pkg.models import AGENT_CONFIGS

        required_fields = ["tools", "mcp_servers", "auto_iflow_tools", "thinking_default"]
    ...
    def test_tools_are_lists(self):
        ...
        for agent_type, config in AGENT_CONFIGS.items():
            ...
            assert isinstance(
                config["auto_iflow_tools"], list
            ), f"Agent '{agent_type}' auto_iflow_tools should be list"
            assert isinstance(
...
class TestGetRequiredMcpServers:
    ...
    def test_coder_has_context7_and_auto_iflow(self):
        """coder should require context7 and auto-iflow."""
        from agents.tools_pkg.models import get_required_mcp_servers

        servers = get_required_mcp_servers("coder")
        assert "context7" in servers
        assert "auto-iflow" in servers
