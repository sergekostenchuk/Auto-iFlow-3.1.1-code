...
class TestGraphitiConfig:
    ...
    def test_from_env_defaults(self, tmp_path):
        ...
        with patch.dict(os.environ, {"HOME": str(tmp_path), "USERPROFILE": str(tmp_path)}, clear=True):
            config = GraphitiConfig.from_env()
            assert config.enabled is False
            assert config.database == "auto_iflow_memory"
            assert "auto-iflow" in config.db_path.lower()  # Default path in ~/.auto-iflow/
