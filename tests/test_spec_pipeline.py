...
mock_init = MagicMock()
mock_init.init_auto_build_dir = MagicMock(return_value=(Path("/tmp"), False))
mock_init.resolve_auto_build_dir = MagicMock(return_value=Path("/tmp"))
...
class TestGetSpecsDir:
    ...
    def test_returns_specs_path(self, temp_dir: Path):
        ...
        with patch('spec.pipeline.models.resolve_auto_build_dir') as mock_resolve:
            mock_resolve.return_value = temp_dir / ".auto-iflow"

            result = get_specs_dir(temp_dir)

            assert result == temp_dir / ".auto-iflow" / "specs"

    def test_calls_init_auto_build_dir(self, temp_dir: Path):
        """Initializes auto-iflow directory."""
        with patch('spec.pipeline.models.init_auto_build_dir') as mock_init:
            mock_init.return_value = (temp_dir / ".auto-iflow", False)

            get_specs_dir(temp_dir, ensure_write=True)
...
class TestSpecOrchestratorInit:
    ...
    def test_init_with_project_dir(self, temp_dir: Path):
        ...
        with patch('spec.pipeline.models.init_auto_build_dir') as mock_init:
            mock_init.return_value = (temp_dir / ".auto-iflow", False)
            specs_dir = temp_dir / ".auto-iflow" / "specs"
    ...
    def test_init_default_model(self, temp_dir: Path):
        ...
        with patch('spec.pipeline.models.init_auto_build_dir') as mock_init:
            ...
            # Default model is glm-4.7 (resolved via model registry if configured)
            assert orchestrator.model == "glm-4.7"
